...
def fetch_sina_a_daily_kline(
    ...
    *,
    ...
    r = requests.get(SINA_HIST_URL.format(sina_symbol), headers=_SINA_HEADERS, timeout=timeout)
    r.raise_for_status()
    ...
    if adjust == "hfq":
        res = requests.get(SINA_HFQ_URL.format(sina_symbol), headers=_SINA_HEADERS, timeout=timeout)
    ...
    if adjust == "qfq":
        res = requests.get(SINA_QFQ_URL.format(sina_symbol), headers=_SINA_HEADERS, timeout=timeout)
        res.raise_for_status()
