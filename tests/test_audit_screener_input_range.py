...
def test_source_diversity_and_delisted_master_gaps_are_informational(monkeypatch):
    ...
    business = Engine([
        ...
        [{
            ...
            "a_buy_amount": 2.0,
            "a_sell_amount": 1.0,
            "a_buy_amount_rate": 2.0,
            "a_sell_amount_rate": 1.0,
            "reason": "history",
        }],
    ...
    monkeypatch.setattr(
        audit_screener_input_range, "create_batch_engine", lambda: business
    )
    monkeypatch.setattr(
        audit_screener_input_range, "get_kline_engine", lambda: kline
    )

    report = audit_screener_input_range.audit_inputs(
        "2026-08-10", "2026-08-10"
    )

    assert report["status"] == "pass"
    assert "unknown_lhb_codes" not in report["hard_failures"]
    assert report["informational"] == {
        "flow_source_count": 2,
        "lhb_codes_absent_from_current_master_and_kline": ["688287"],
    }
