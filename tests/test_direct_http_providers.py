...
from acquisition.normalize import normalize_batch
...
def finance_row(**overrides):
    return {"SECURITY_CODE": "000001", "SECUCODE": "000001.SZ",
            ...
            "NOTICE_DATE": "2026-08-20 00:00:00", "UPDATE_DATE": "2026-09-04 12:34:56",
...
def test_notice_accepts_native_colon_millisecond_display_time():
    result, _, _ = fetch(request("notices"), notice_page([
        notice(display_time="2026-09-04 19:02:03:816")]))
    row = result["outcomes"]["000001.SZ"]["rows"][0]
    assert row["display_time"] == "2026-09-04T19:02:03.816000+08:00"
    assert row["source_time"] == row["display_time"]
...
def test_finance_preserves_identity_dates_and_raw_precision_once():
    ...
    raw = finance_row()
    result, client, _ = fetch(req, *finance_responses(page([raw])))
    row = result["outcomes"]["000001.SZ"]["rows"][0]
...
def test_native_json_decimal_token_is_not_rounded_through_binary_float():
    ...
    response._content = (
        ...
        b'"NOTICE_DATE":"2026-08-20","UPDATE_DATE":"2026-09-04 12:00:00",'
        b'"TOTALOPERATEREVE":123456789012345.678901}]}}'
    )
    result, _, _ = fetch(request(), *finance_responses(response))
    row = result["outcomes"]["000001.SZ"]["rows"][0]
...
@pytest.mark.parametrize("change,code", [
    ...
    ({"SECUCODE": "000002.SZ"}, "SECURITY_MISMATCH"),
...
def test_finance_rejects_wrong_issuer_or_date(change, code):
    result, _, _ = fetch(request(), *finance_responses(page([finance_row(**change)])))
    assert result["outcomes"]["000001.SZ"]["error_code"] == code


def test_finance_complete_date_discovery_proves_no_change_for_an_issuer():
    result, _, _ = fetch(request(), page([]), page([]))
    assert result["outcomes"]["000001.SZ"] == {"status": "no_data", "rows": [], "reason": "empty_event_set"}
    assert normalize_batch(get_spec("finance"), result).units[0].status == "no_data"


def test_finance_maps_eastmoney_nq_suffix_to_project_bj_symbol():
    raw = finance_row(SECURITY_CODE="832010", SECUCODE="832010.NQ")
    result, _, _ = fetch(request(codes=["832010.BJ"]), *finance_responses(page([raw])))
    assert result["outcomes"]["832010.BJ"]["rows"][0]["symbol"] == "832010.BJ"


def test_finance_eps_diluted_and_nonrecurring_metrics_are_not_swapped():
    result, _, _ = fetch(request(), *finance_responses(page([finance_row(EPSXS="1.01", EPSKCJB="0.92")])))
    row = result["outcomes"]["000001.SZ"]["rows"][0]
...
def test_finance_reads_every_page_and_checks_total(monkeypatch):
    ...
    result, client, _ = fetch(request(), *finance_responses(
        page([finance_row()], pages=2, count=2),
        page([finance_row(REPORT_DATE="2026-03-31", REPORT_TYPE="一季报")], pages=2, count=2)))
    assert len(result["outcomes"]["000001.SZ"]["rows"]) == 2
    assert [call[1]["params"]["p"] for call in client.calls] == [1, 1, 2]
...
def test_partial_or_changed_report_never_returns_data(second, error):
    result, _, _ = fetch(request(), *finance_responses(page([finance_row()], pages=2, count=2), second()))
...
def test_page_limit_is_error_not_success(monkeypatch):
    ...
    result, client, _ = fetch(request(), *finance_responses(page([finance_row()], pages=2, count=2)))
    assert result["outcomes"]["000001.SZ"]["error_code"] == "PAGINATION_LIMIT"
    assert len(client.calls) == 2


def test_http_provider_does_not_offer_capital_flow_fallback():
    result, client, _ = fetch(request("capital_flow_daily"))
    assert result["outcomes"]["000001.SZ"]["error_code"] == "INVALID_REQUEST"
    assert client.calls == []
...
def test_billboard_detail_keeps_both_sides_reasons_and_trade_identity():
    ...
    assert client.calls[0][1]["params"]["filter"] == "(TRADE_DATE='2026-09-04')"


def test_billboard_special_seats_with_same_zero_code_remain_distinct():
    first = alist_row(OPERATEDEPT_CODE="0", OPERATEDEPT_NAME="机构专用")
    second = alist_row(OPERATEDEPT_CODE="0", OPERATEDEPT_NAME="机构专用",
                       BUY="30.12", SELL="11.11", NET="19.01")
    result, _, _ = fetch(request("alist_detail"), page([first, second]), page([]))
    rows = result["outcomes"]["000001.SZ"]["rows"]
    assert [row["a_buy_amount"] for row in rows] == ["20.12", "30.12"]


def test_billboard_date_result_is_fetched_once_then_partitioned_from_cache():
    timer = Clock()
    rows = [alist_row(), alist_row(SECURITY_CODE="000002", SECUCODE="000002.SZ", TRADE_ID="107")]
    client = Client(page(rows))
    provider = EastmoneyProvider(client, timer, timer.sleep)
    first = provider.fetch_batch("alist_daily", request("alist_daily"))
    second_request = request("alist_daily", codes=["000002.SZ"])
    second_request["request_id"] = "batch-002"
    second = provider.fetch_batch("alist_daily", second_request)
    assert first["outcomes"]["000001.SZ"]["status"] == "data"
    assert second["outcomes"]["000002.SZ"]["status"] == "data"
...
def test_global_notice_without_requested_association_is_legal_no_data():
    result, _, _ = fetch(request("notices"), notice_page([notice(codes=[{"stock_code": "000002"}])]))
    assert result["outcomes"]["000001.SZ"]["status"] == "no_data"
...
def test_non_listed_notice_association_is_ignored_without_poisoning_the_date():
    result, _, _ = fetch(request("notices"), notice_page([
        notice(codes=[{"stock_code": "A25209"}])]))
    assert result["outcomes"]["000001.SZ"]["status"] == "no_data"
...
def test_retry_after_is_respected(retry_after):
    result, client, clock = fetch(request(), Response(status=429, headers={"Retry-After": retry_after}),
                                  page([]), page([finance_row()]))
    assert result["outcomes"]["000001.SZ"]["status"] == "data"
    assert clock.sleeps == [17] and len(client.calls) == 3
