...
def test_minute_batch_cap_applies_to_inherited_runtime(configured, expected):
    with patch("tools.sync_qmt_primary.build_child_env", return_value={
        ...
        "BIG_QMT_MINUTE_BATCH_SIZE": configured,
    }), patch("tools.sync_qmt_primary.subprocess.run", return_value=SimpleNamespace(returncode=75)) as run:
        result = sync_qmt_primary.run_dataset("minute_price")
    assert run.call_args.kwargs["env"]["QMT_PRODUCTION_MINUTE_BATCH_SIZE"] == expected
    assert run.call_args.kwargs["env"]["BIG_QMT_MINUTE_BATCH_SIZE"] == expected
    assert result["status"] == "failed"
    assert result["error_code"] == "QMT_HISTORY_RESOURCE_PRESSURE"
