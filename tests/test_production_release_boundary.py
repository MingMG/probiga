...
def test_final_governance_api_and_page_smoke_is_fail_closed_before_receipt():
    ...
    for required in (
        ...
        "APP_NAV",
        "renderSidebar",
        "strategy governance dynamic navigation entry missing",
        '"$PREPARED_CODE_ROOT/server/static/index.html"',
        '"$PREPARED_CODE_ROOT/server/static/js/app.js"',
...
def test_production_deploy_pins_scheduler_flag_in_execstart() -> None:
    ...
    assert "--workers" not in main_dropin
    assert "--limit-max-requests" not in main_dropin
    assert 'test "${MAIN_CMDLINE[8]}" = 127.0.0.1' in deploy_script
    assert 'test "${MAIN_CMDLINE[10]}" = 8000' in deploy_script
    assert 'test "${#MAIN_CMDLINE[@]}" -eq 11' in deploy_script
    assert (
