...
def test_portfolio_mutations_reconcile_without_forcing_an_uncached_read():
    js = (ROOT / "server" / "static" / "js" / "app.js").read_text(encoding="utf-8")
    ...
    mutation_reload = js.split("function pfReloadAfterMutation()", 1)[1].split(
        "window.pfAdd = function", 1
    )[0]
    assert "force: false" in mutation_reload


def test_portfolio_load_retries_transient_outages_and_keeps_rendered_data():
    js = (ROOT / "server" / "static" / "js" / "app.js").read_text(encoding="utf-8")
    portfolio_js = js.split("portfolio: function", 1)[1].split("datasource: function", 1)[0]

    assert "function pfFetchPortfolioWithRetry(force)" in portfolio_js
    assert "[0, 1000, 2000, 4000, 8000, 15000, 15000, 15000, 10000]" in portfolio_js
    assert "var retryDeadline = Date.now() + 75000" in portfolio_js
    assert "pfIsTransientRequestError" in portfolio_js
    assert "服务短暂切换，正在自动重连" in portfolio_js
    assert "服务暂不可用，已保留上次数据" in portfolio_js
    assert "pfFetchPortfolioWithRetry(!!options.force)" in portfolio_js
    assert "portfolioLoadToken !== window._pfLoadToken" in portfolio_js
    assert "activeTabId() !== 'portfolio'" in portfolio_js
    assert "function pfWaitForRetry(waitMs)" in portfolio_js
    assert "payload.retryable === false ? 400 : 503" in portfolio_js
    assert "pfLiveUrl(!!force, forceRefreshId)" in portfolio_js
    assert "&refresh_id=" in portfolio_js
    assert "Date.now() + waitMs >= retryDeadline" in portfolio_js
    assert "Math.min(12000, remainingMs)" in portfolio_js
    assert "window.pfFetchPortfolioWithRetry = pfFetchPortfolioWithRetry" in portfolio_js
    assert "window.pfFetchPortfolioWithRetry(true)" in js
    assert "error.invalidJson && (!status || status === 200)" in portfolio_js
    assert "if (keepCurrent && tabId !== 'portfolio')" in js
