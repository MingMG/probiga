...
数据库 `cutover` / `resume` 失败时，诊断另外提供固定枚举的执行子阶段、异常类别，以及可信异常因果链中的 MySQL 整数错误码。错误码可能来自被恢复失败包装的先前操作，不单独证明当前阶段的根因。不会输出异常原文、SQL、参数或连接凭据；`preflight` 的严格输出合同保持不变。这些字段只帮助定位失败，不重试数据库操作，也不改变恢复或授权条件。
...
`socks5` 使用本机 DNS，`socks5h` 使用代理端 DNS。两端解析结果或到目标地址的连通性可能不同，诊断时需要分别核验；任何路径都保留 TLS 证书校验。
