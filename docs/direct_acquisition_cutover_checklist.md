...
- 已确认盘中资金流采集没有请求供应商 `f124`，而页面以本地采集分钟判断新鲜，可能使旧行情触发新提醒。本次复用已存在的 `source_time/received_at/data_source` 列：缺失、跨日、未来或超过 180 秒的源数据不进入当前快照，自选股按源时间计算窗口。41 项相关回归通过；真实 5 条周末样本的源时间均为 9 月 4 日，被新规则拒绝标新。保留原 98% 覆盖门槛，交易时段持续供数仍需实测。
- 开盘准备只读核对：9 月 4 日日线 5546 只、无非法收盘价，日资金流 5207 只、五字段无缺失；盘中表 5545 只、343790 行，最后时间 14:58。9 月 7—11 日历均开市，现有盘中资金流任务每 2 分钟、两条盘后任务均启用。
- 用户继续授权后，16:50 的系统 UAC 只读预检已取得真实管理员令牌，两项既有 Windows 任务可读且原状态均 Enabled/Ready；该预检没有改变任务或数据库。
- 新增一次性 `tools/resume_qmt_prior_edge.ps1`，仅恢复仍被授权和选择的旧生产版本，再交给现有受控发布。必须从已合并的精确 main 在管理员会话运行；先核验自身真实 PID/运行心跳，再生成本次新 bootstrap 回执，拒绝历史 idempotent 回执，按保护上下文优雅停机并恢复任务原状态。24 项真实 PowerShell 5 回归及独立审查通过；脚本存在不等于生产已执行成功，实际结果以恢复和部署日志为准。
