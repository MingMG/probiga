# -*- coding: utf-8 -*-
"""
聚宽Notebook简易版 - 直接复制粘贴到聚宽Notebook运行
===================================================

使用方法：
1. 登录 https://www.joinquant.com
2. 进入"我的研究" > 新建 Notebook
3. 将下面的代码复制粘贴到Notebook中
4. 点击"运行"即可
"""
...
for stock in stock_list[:100]:  # 限制数量避免超时，实际可去掉
    try:
        df = get_price(stock, start_date=start_date, end_date=today, frequency='daily')
        if len(df) < 20:
            continue
...
try:
    ...
    if result.get('success'):
        print(f"\n✅ 同步成功！策略: {STRATEGY_NAME}, 股票数: {result.get('count')}")
    else:
