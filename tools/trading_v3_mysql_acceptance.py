...
def run_mysql_acceptance(
    ...
    *,
    ...
    expected_uuid = require_expected_server_uuid(expected_server_uuid)
    if mode not in {"serial-replay", "concurrent-initial", "partial-recovery"}:
    ...
    if type(partial_statement_count) is not int or not (
    ...
    try:
        ...
        if mode == "concurrent-initial":
            ...
            expected = tuple(
                "applied"
                if str(item["version"]) == V3_PROJECTION_OUTBOX_MIGRATION_VERSION
                else "exists"
                for item in MIGRATIONS
            )
    ...
    return V3MySQLAcceptanceReport(
        ...
        partial_committed_statement_count=(
            partial_statement_count if mode == "partial-recovery" else 0
        ),
...
def _parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--mode",
        choices=("serial-replay", "concurrent-initial", "partial-recovery"),
