...
def _run_on_production_host() -> int:
    ...
    try:
        command = (
            ...
            "tools/verify_trading_v3_production.py --local-runtime"
        )
...
def main() -> int:
    ...
    try:
        ...
        current_version = str(load_v3_config()["strategy_version"])
        ...
        checklist = {
            ...
            "database_real_trade_guards_present": (
                all(guard_status.values()) and len(guards) == 4
            ),
            ...
            "counterfactual_drain_scheduler_ready": (
                ...
                and any(
                    ...
                    and "--max-batches" in str(
                        item.get("script_args") or ""
                    )
                    for item in counterfactual_tasks
                )
            ),
