"""Self-contained internal hash validator for the frozen V6 release.
...
SOURCE_PREFIXES = ("server/trading_v6/", "tools/trading_v6/")


class V6ReleaseIntegrityError(ValueError):
    """Raised when any byte-owned V6 release contract drifts."""


@dataclass(frozen=True, slots=True)
class V6ReleaseIntegrity:
...
def validate_v6_release() -> V6ReleaseIntegrity:
    ...
    _exact(document, "system", "trading_v6")
    _exact(document, "system_version", "6.0.0-research")
    ...
    if not source_files:
        raise V6ReleaseIntegrityError("source_files must not be empty")
    discovered = {
        path.relative_to(ROOT).as_posix()
        for relative_root in (ROOT / "server/trading_v6", ROOT / "tools/trading_v6")
        for path in relative_root.rglob("*.py")
        if path.is_file()
    }
    if set(source_files) != discovered:
        raise V6ReleaseIntegrityError(
    ...
    for relative_path, digest in source_files.items():
        ...
        if prefix is None:
            raise V6ReleaseIntegrityError("source path crosses V6 ownership")
    ...
    if config_files.get(RUNTIME_RELATIVE_PATH) != config_hash:
        raise V6ReleaseIntegrityError("runtime hash is absent from config_files")
    config_root = f"strategies/trading_v6/releases/{RELEASE_ID}"
    discovered_configs = {
        path.relative_to(ROOT).as_posix()
    ...
    for relative_path, digest in config_files.items():
        if not relative_path.startswith(config_root + "/"):
            raise V6ReleaseIntegrityError("config crosses the release namespace")
    ...
    if not isinstance(evidence_files, Mapping) or set(evidence_files) != {
        ...
        raise V6ReleaseIntegrityError("historical evidence set differs")
    for name, raw_item in evidence_files.items():
        ...
        if not relative_path.startswith("artifacts/trading_v6/"):
            raise V6ReleaseIntegrityError("evidence crosses V6 ownership")
        _verify_owned_hash(
            relative_path,
    ...
    dependencies = document.get("historical_dependency_files")
    if not isinstance(dependencies, Mapping) or set(dependencies) != {
        "legacy_runner"
    }:
        raise V6ReleaseIntegrityError("historical dependency set differs")
    runner = _mapping(dependencies["legacy_runner"], "legacy runner")
    _exact(runner, "decision", "BLOCK")
    _exact(runner, "current_source_reproducible", False)
    _exact(runner, "path", "tools/research_trading_v4_ml_campaign.py")
    _verify_owned_hash(
        runner["path"],
        _digest(runner.get("sha256"), "legacy runner digest"),
        "tools",
    )

    parent_id = "trading_v5.0.0-research"
    parent_path = (
    ...
    parent_file = _owned_file(parent_path, "versions/trading_v5/releases")
    if _sha256(parent_file.read_bytes()) != parent_digest:
        raise V6ReleaseIntegrityError("parent manifest byte hash differs")
    parent = _strict_json(parent_file.read_bytes(), "parent manifest")
    _exact(parent, "system", "trading_v5")
    _exact(parent, "release_id", parent_id)

    _exact(document, "entrypoint", "tools/trading_v6/audit_release.py")
    if document["entrypoint"] not in source_files:
        raise V6ReleaseIntegrityError("entrypoint is not source-owned")
    ...
    _exact(document, "historical_candidate_count", 3)
    _exact(document, "full_stress_matrix_executed", False)
    _exact(document, "performed_bootstrap_iterations", 0)
    _exact(document, "registered_model_count", 0)
    return V6ReleaseIntegrity(
        document=document,
...
def _validate_runtime(runtime: Mapping[str, Any]) -> None:
    ...
    if database != {
        ...
        raise V6ReleaseIntegrityError("runtime database evidence is not deferred")
    ...
    if execution != {
        ...
        raise V6ReleaseIntegrityError("runtime execution boundary is not closed")
    ...
    limitations = _mapping(
        runtime.get("historical_limitations"),
        "runtime historical_limitations",
    )
    _exact(limitations, "stress_matrix_complete", False)
    _exact(limitations, "historical_success_cannot_activate", True)
    primitives = _mapping(
        runtime.get("research_primitives"),
        "runtime research_primitives",
    )
    _exact(primitives, "historical_result_reproduced_by_new_primitives", False)
    _exact(primitives, "registered_model_count", 0)
    _exact(primitives, "explicit_input_source_certified", False)
    _exact(primitives, "trading_calendar_snapshot_bound", False)
    _exact(primitives, "freshness_sla_enforced", False)
    _exact(primitives, "minimum_peer_coverage_enforced", False)
...
def _strict_json(payload: bytes, label: str) -> Mapping[str, Any]:
    def no_duplicates(pairs: list[tuple[str, Any]]) -> dict[str, Any]:
        ...
        for key, value in pairs:
            if key in result:
                raise V6ReleaseIntegrityError(f"{label} duplicate key: {key}")
            result[key] = value
        return result

    def reject_constant(value: str) -> None:
        raise V6ReleaseIntegrityError(f"{label} contains non-finite {value}")
    ...
    try:
        ...
        raise V6ReleaseIntegrityError(f"{label} is not strict UTF-8 JSON") from exc
...
def _reject_non_finite(value: Any, path: str) -> None:
    if isinstance(value, float) and not math.isfinite(value):
        raise V6ReleaseIntegrityError(f"{path} contains a non-finite number")
...
def _digest_mapping(value: Any, label: str) -> dict[str, str]:
    ...
    for raw_path, raw_digest in mapping.items():
        ...
        if canonical in result:
            raise V6ReleaseIntegrityError(f"{label} has a canonical path collision")
...
def _verify_owned_hash(relative_path: str, digest: str, owner: str) -> None:
    ...
    if _sha256(path.read_bytes()) != digest:
        raise V6ReleaseIntegrityError(f"release file hash drifted: {relative_path}")
...
def _owned_file(relative_path: str, owner: str) -> Path:
    ...
    if not path.is_file():
        raise V6ReleaseIntegrityError(f"release file is missing: {relative_path}")
    return path
...
def _reject_reparse_points(path: Path, owner_path: Path) -> None:
    ...
    while True:
        ...
        if owner_path not in current.parents:
            raise V6ReleaseIntegrityError("release ownership parent chain differs")
        current = current.parent


def _canonical_path(value: Any, label: str) -> str:
    if not isinstance(value, str) or not value or value != value.strip():
        raise V6ReleaseIntegrityError(f"{label} must be canonical text")
    if "\\" in value or "//" in value or "/./" in value:
        raise V6ReleaseIntegrityError(f"{label} is not canonical")
    path = PurePosixPath(value)
    if path.is_absolute() or ".." in path.parts or path.as_posix() != value:
        raise V6ReleaseIntegrityError(f"{label} escapes or is not canonical")
    return value


def _mapping(value: Any, label: str) -> Mapping[str, Any]:
    if not isinstance(value, Mapping):
        raise V6ReleaseIntegrityError(f"{label} must be an object")
    return value


def _text(value: Any, label: str) -> str:
    if not isinstance(value, str) or not value.strip():
        raise V6ReleaseIntegrityError(f"{label} must be non-empty text")
    return value
...
def _digest(value: Any, label: str) -> str:
    if (
        ...
        raise V6ReleaseIntegrityError(f"{label} must be a lowercase SHA-256")
    return value
