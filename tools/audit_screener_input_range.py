...
and listing-date consistency. Codes absent from the *current* master are
reported separately because valid delisted history must not fail the audit.
"""
...
def audit_inputs(
    ...
    *,
    ...
    # Multiple named providers are expected in a historical backfill and are
    # not a consistency defect.  Likewise, a fact row can legitimately refer
    # to a delisted stock that no longer exists in the current master table.
    # Keep both visible without turning complete history into a false FAIL.
    informational = {
        "flow_source_count": len(source_counts),
        "lhb_codes_absent_from_current_master": sorted(unknown_lhb_codes),
    }
    failed = any(bool(value) for value in hard_failures.values())
    return {
