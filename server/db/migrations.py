...
def _run_column_collation_migrations(conn, *, dry_run: bool) -> list[MigrationResult]:
    ...
    for item in REQUIRED_COLUMN_COLLATIONS:
        ...
        if not _table_exists(conn, table_name):
            results.append(
                MigrationResult(
                    table_name, column_name, "missing_table", "table does not exist"
                )
            )
            continue
        ...
        if not metadata:
            results.append(
                MigrationResult(
                    table_name, column_name, "missing_column", "column does not exist"
                )
            )
            continue
        ...
        if (
            str(metadata.get("CHARACTER_SET_NAME") or "").lower()
            == expected_charset.lower()
            and str(metadata.get("COLLATION_NAME") or "").lower()
            ...
            results.append(
                MigrationResult(
                    ...
                    column_name,
                    "exists",
                    "column collation already matches",
                )
            )
            continue
        ...
        # the validated contract guarantees type/null/default/index semantics.
        conn.execute(
            text(
        ...
        if (
            str(verified.get("CHARACTER_SET_NAME") or "").lower()
            != expected_charset.lower()
            or str(verified.get("COLLATION_NAME") or "").lower()
            != expected_collation.lower()
        ):
            raise RuntimeError(
    ...
    with engine.begin() as conn:
        for item in REQUIRED_COLUMNS:
            ...
            if _column_exists(conn, table_name, column_name):
                results.append(
                    MigrationResult(
                        table_name, column_name, "exists", "column already exists"
                    )
                )
                continue
            ddl = item["ddl"]
        ...
        for item in REQUIRED_UNIQUE_INDEXES:
            ...
            if not _table_exists(conn, table_name):
                results.append(
                    MigrationResult(
                        table_name, index_name, "missing_table", "table does not exist"
                    )
                )
                continue
            if _index_exists(conn, table_name, index_name):
                results.append(
                    MigrationResult(
                        table_name, index_name, "exists", "index already exists"
                    )
                )
                continue
            ddl = (
            ...
            if dry_run:
                ...
                continue
            conn.execute(
                text(f"ALTER TABLE {quote_identifier(table_name)} ADD {ddl}")
            )
...
def summarize_results(results: list[MigrationResult]) -> dict[str, Any]:
    ...
    for result in results:
        summary["by_status"][result.status] = (
            summary["by_status"].get(result.status, 0) + 1
        )
    return summary
