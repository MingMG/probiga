(function(){
  ...
  function renderUnifiedCandidates(){
    ...
    el('unifiedCandidateSummary').innerHTML=[
      fact('数据日期',run.data_date||'—'),fact('数据新鲜度',run.freshness==='live'?'实时':run.freshness==='historical_close'?'收盘复盘':status(run.freshness)),fact('候选等级','A '+(grades.A||0)+' / B '+(grades.B||0)+' / C '+(grades.C||0)+' / 拒绝 '+(grades.REJECT||0)),
  ...
  function renderPositions(){var names={},summary=(state.paperLedger||{}).summary||{};state.forecasts.forEach(function(x){names[x.stock_code]=x.short_name});state.targets.forEach(function(x){names[x.stock_code]=x.short_name});el('positionSummary').innerHTML=[fact('当前持仓',String(summary.position_count||state.positions.length)+' 笔'),fact('合计持仓市值',money(summary.current_market_value)),fact('合计浮动盈亏',money(summary.total_unrealized_pnl)),fact('数据说明','按每只股票最新生产行情逐笔计算，未取得行情时明确显示暂无行情')].join('');el('positionRows').innerHTML=state.positions.length?state.positions.map(function(x){var source=x.ledger_source==='LEGACY_EVENT_SIM'?'事件模拟账本':'V2/V3账本',pnl=Number(x.unrealized_pnl),pnlClass=x.unrealized_pnl==null?'':Number.isFinite(pnl)?(pnl>0?'safe-text':pnl<0?'danger-text':''):'',quote=x.current_price==null?'暂无行情':money(x.current_price),quoteAt=x.quote_at?String(x.quote_at)+' · '+sourceName(x.quote_source):'暂无行情时间';return '<tr><td>'+security(x.stock_code,x.short_name||names[x.stock_code])+'</td><td>'+esc(status(x.position_state||x.state))+'</td><td>'+num(x.remaining_quantity||x.quantity,0)+'</td><td>'+num(x.sellable_quantity,0)+'</td><td>'+money(x.cost_price||x.average_cost)+'</td><td>'+quote+'</td><td>'+money(x.market_value)+'</td><td class="'+pnlClass+'">'+money(x.unrealized_pnl)+'</td><td class="'+pnlClass+'">'+(x.unrealized_pnl_pct==null?'—':pct(x.unrealized_pnl_pct))+'</td><td>'+money(x.protective_stop)+'</td><td>'+num(x.add_count,0)+'</td><td>'+esc(quoteAt)+'</td><td class="reason">'+esc('['+source+'] '+(x.invalidation_condition||x.last_reason||'趋势、硬止损和净期望动态复核'))+'</td></tr>'}).join(''):empty(13,'当前统一模拟账本没有持仓；候选不是持仓，只有模拟成交后才会显示')}
