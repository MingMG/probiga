(function(){
  'use strict';
  var state={readiness:{},overview:{},forecasts:[],unifiedRun:{},oversold:[],targets:[],validation:null,recall:null,learning:{},account:{},paperLedger:{},positions:[],orders:[],runs:[],intraday:{},hypotheses:[],hypothesisDetail:null,tasks:[],dataEvidence:{},candidatePage:1,actionMessage:''};
  ...
  function renderValidation(){
    ...
    [['activeOosGate',activePass,activePass?models.length+' 个':'BLOCK'],['validationGate',matchingPass,matchingPass?'PASS':'BLOCK'],['level1Gate',level1Pass,level1Days+' / 5 日']].forEach(function(item){var node=el(item[0]);node.textContent=item[2];node.className=item[1]?'safe-text':'danger-text'});
    var passed=[activePass,matchingPass,level1Pass].filter(Boolean).length;
    el('criticalAcceptanceStatus').textContent=passed+' / 3 PASS';el('criticalAcceptanceStatus').className='badge '+(passed===3?'safe':'danger');
    el('criticalAcceptanceFacts').innerHTML=[
      ...
      fact('Level1 连续交易日',level1Days+' / 5；状态 '+(level1.status||'BLOCK')),
      fact('真实下单',ready.real_trading_enabled?'异常：已开启':'固定关闭')
    ].join('');
    el('oosSamples').textContent=num(v.sample_count,0);el('oosExpectancy').textContent=pct(v.net_expectancy_pct);el('oosPf').textContent=ratio(v.profit_factor);el('oosPayoff').textContent=ratio(v.payoff_ratio);el('oosDd').textContent=pct(v.maximum_drawdown_pct);var evidence=v.evidence||{},p=evidence.portfolio||{};el('oosProfit').textContent=money(p.net_profit_cny);el('validationStatus').textContent=v.result_status||'暂无结果';el('validationStatus').className='badge '+(v.result_status==='PASS'?'safe':'danger');el('validationEvidence').innerHTML=[fact('模型版本',v.model_version||'—'),fact('样本外区间',(v.period_start||'—')+' 至 '+(v.period_end||'—')),fact('阻断原因',(v.block_reasons||[]).join('；')||'无'),fact('最终权益',money(p.final_equity_cny)),fact('组合收益',pct(p.total_return_pct)),fact('交易次数',p.trade_count==null?'—':p.trade_count)].join('')
  }
  ...
  function reloadCandidates(){state.candidatePage=1;var day=el('candidateDate').value,sf=el('strategyFilter').value,st=el('statusFilter').value,q=el('candidateSearch').value.trim(),params=['limit='+(q?1000:200)];if(day)params.push('trade_date='+encodeURIComponent(day));if(sf)params.push('strategy_key='+encodeURIComponent(sf));if(st)params.push('status='+encodeURIComponent(st));if(q)params.push('q='+encodeURIComponent(q));Promise.all([
    ...
    postJson('/api/screener/run',{preset:day?'capital_support':'intraday_sector',as_of_date:day,universe:'market',top:100,filters:{exclude_st:true}}).then(function(v){state.unifiedRun=v}).catch(function(err){state.unifiedRun={status:'error',error:err.message,data:[],stats:{}}})
