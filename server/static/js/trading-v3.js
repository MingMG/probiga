(function(){
  'use strict';
  var state={dailyResult:{},dailyDelivery:{},context:{},marketClock:{},readiness:{},overview:{},stockPool:{},researchStockPool:{},researchStockPoolRequestDate:'',requestedDateMode:'DEFAULT',candidateRefreshSeq:0,candidateRefreshPending:false,auctionGate:{},watchlistStrategy:{},forecasts:[],oversold:[],targets:[],validation:null,recall:null,learning:{},account:{},paperLedger:{},positions:[],orders:[],runs:[],intraday:{},hypotheses:[],hypothesisDetail:null,tasks:[],dataEvidence:{},lineage:{},researchGovernance:{},batchDiff:{},decisionIntelligence:{},horizonValidation:{},counterfactualResearch:{},shadowPreview:{},strategyVersions:[],researchOperations:{},activeBacktest:{},compareBacktests:[],formalBacktestMessage:'',researchErrors:{},candidatePage:1,researchCandidatePage:1,actionMessage:'',errors:{},staleKeys:{},loadedKeys:{},pendingKeys:{},requestedDate:'',activeView:'overview',filters:{},activeJobId:'',activeBacktestJobId:'',loadSeq:0,loading:true,truthLoading:true};
  var titles={overview:'今日策略',hypotheses:'连续跟踪',candidates:'策略选股',intraday:'盘中应急',portfolio:'目标组合',positions:'我的持仓',orders:'模拟订单',validation:'策略研究',missed:'漏抓复盘',evidence:'数据与系统'};
  var subtitles={overview:'先处理持仓，再看新机会',positions:'自选股里的真实持仓，今天怎么操作',candidates:'看策略今天选出了什么，以及为什么入选或阻断',intraday:'盘中出现意外，先执行退出红线',hypotheses:'买入以后继续跟，不让建议断档',validation:'比较精确策略版本，区分研究回测、样本外验收和模拟表现'};
  ...
  function metricNumber(v){if(v==null||typeof v==='boolean'||typeof v==='object'||typeof v==='string'&&!v.trim())return null;var n=Number(v);return Number.isFinite(n)?n:null}
  function num(v,d){var n=metricNumber(v);return n==null?'—':n.toFixed(d==null?2:d)}
  function pct(v,d){var n=metricNumber(v);return n==null?'—':n.toFixed(d==null?2:d)+'%'}
  function fractionPct(v,d){var n=metricNumber(v);return n==null?'—':pct(n*100,d)}
  function ratio(v){return num(v,2)}
  function money(v){var n=metricNumber(v);return n==null?'—':'¥'+n.toLocaleString('zh-CN',{minimumFractionDigits:2,maximumFractionDigits:2})}
  function displayDateTime(v){if(v==null||v==='')return '—';return String(v).replace('T',' ').replace(/\.000000$/,'').replace(/\.000$/,'')}
  function firstNumber(values){for(var i=0;i<values.length;i+=1){var n=metricNumber(values[i]);if(n!=null)return n}return null}
  ...
  function responseError(r,path){return r.text().then(function(body){var safeMessage='';try{var parsed=JSON.parse(body);safeMessage=parsed.message||parsed.error||(typeof parsed.detail==='string'?parsed.detail:'')}catch(ignore){}throw new Error('HTTP '+r.status+(safeMessage?' · '+safeMessage:'')+' · '+path)})}
  ...
  function postJsonBody(path,body){return fetch(path,{method:'POST',headers:{Accept:'application/json','Content-Type':'application/json'},body:JSON.stringify(body)}).then(function(r){if(!r.ok)return responseError(r,path);return r.json()})}
  ...
  function security(code,name){code=String(code||'').split('.')[0];name=String(name||code||'—');if(!/^\d{6}$/.test(code))return esc(name);var href='/?tab=workbench&stock_code='+encodeURIComponent(code);return '<span class="security"><a class="name" href="'+href+'" data-stock-code="'+esc(code)+'" data-stock-name="'+esc(name)+'">'+esc(name)+'</a><a class="code" href="'+href+'" data-stock-code="'+esc(code)+'" data-stock-name="'+esc(name)+'">'+esc(code)+'</a></span>'}
  function requestStockChart(code,name){
    code=String(code||'').split('.')[0];name=String(name||code||'');
    if(!/^\d{6}$/.test(code))return;
    if(window.parent&&window.parent!==window){parentMessage('probiga-open-stock-detail',{stock_code:code,short_name:name});return}
    window.location.assign('/?tab=workbench&stock_code='+encodeURIComponent(code))
  }
  ...
  function hasCalibratedExpectation(x){
    ...
    return uncalibrated.indexOf(String(x.forecast_status||''))<0&&(Number(x.sample_count||0)>0||statuses.indexOf('VALIDATED_POSITIVE')>=0)&&metricNumber(x.expected_return_net_pct)!=null;
  }
  ...
  function hypothesisEstimate(value,kind){var n=metricNumber(value);if(n==null||n<0||n>1)return '—';return kind==='OOS_CALIBRATED'?fractionPct(n,1)+' · 历史校准':num(n*100,1)+' / 100 · 研究评分'}
  ...
  function stockPoolIsReadable(pool){
    ...
    var sessionDate=String(pool.decision_date||pool.decision_session_date||'').slice(0,10),dataDate=String(pool.trade_date||pool.data_date||'').slice(0,10),executionDate=String(pool.execution_session_date||'').slice(0,10);
    ...
    return !!pool.run_uid&&pool.pool_readable===true&&runStatus==='COMPLETED'&&pool.decision_integrity_verified===true&&(poolStatus==='READY'||poolStatus==='EMPTY')&&datePattern.test(sessionDate)&&datePattern.test(dataDate)&&datePattern.test(executionDate)&&dataDate===sessionDate&&executionDate>sessionDate&&Array.isArray(items)&&Number.isInteger(stockCount)&&stockCount===items.length&&Number.isInteger(candidateCount)&&candidateCount===actualCandidateCount&&((poolStatus==='READY'&&candidateCount>0)||(poolStatus==='EMPTY'&&candidateCount===0));
  }
  function stockPoolFormalTruth(pool,requestedDate,latestFormalDate){
    pool=pool||{};var datePattern=/^\d{4}-\d{2}-\d{2}$/,decisionDate=String(pool.decision_date||pool.decision_session_date||'').slice(0,10),dataDate=String(pool.trade_date||pool.data_date||'').slice(0,10),executionDate=String(pool.execution_session_date||'').slice(0,10),target=String(requestedDate||pool.requested_trade_date||decisionDate||'').slice(0,10),latestDate=String(latestFormalDate||'').slice(0,10),asOf=pool.is_as_of_fallback===true,reasonCodes=Array.isArray(pool.reason_codes)?pool.reason_codes.filter(Boolean).join('；'):'';
    function blocked(reason,code){return {ready:false,verifiedCompleted:false,requestedDate:target,decisionDate:decisionDate,dataDate:dataDate,executionDate:executionDate,reason:reason,reasonCode:code||'FORMAL_POOL_BLOCKED'}}
    ...
    if(!datePattern.test(decisionDate))return blocked('策略池缺少有效决策日','POOL_DATE_MISMATCH');
    ...
    if(!datePattern.test(executionDate)||executionDate<=decisionDate)return blocked('策略池缺少决策日严格下一交易日的执行会话映射','POOL_EXECUTION_SESSION_INVALID');
    ...
    if(String(pool.decision_scope||'').toUpperCase()==='RESEARCH_ONLY')return blocked('批次权限为 RESEARCH_ONLY，不能升级为当前可执行票池','RESEARCH_ONLY');
    var canonicalHash=String(pool.canonical_result_hash||''),canonicalGovernance=String(pool.source_system||'').toUpperCase()==='STRATEGY_GOVERNANCE'&&String(pool.decision_scope||'').toUpperCase()==='CANONICAL_GOVERNANCE'&&canonicalHash.length===64&&pool.decision_integrity_verified===true&&pool.real_order_authority===false;
    if(!canonicalGovernance)return blocked('当前批次不是带规范哈希的 STRATEGY_GOVERNANCE 结果，不能升级为正式票池','NOT_CANONICAL_GOVERNANCE');
    return {ready:true,verifiedCompleted:true,requestedDate:target,decisionDate:decisionDate,dataDate:dataDate,executionDate:executionDate,reason:asOf?'请求日使用不晚于该日的最新成功批次':'身份、日期与完整性均已通过',reasonCode:asOf?'VERIFIED_COMPLETED_LATEST_AS_OF_POOL':'VERIFIED_COMPLETED_CURRENT_POOL'}
  }
  function normalizedRequestedDateMode(value){return String(value||'').toUpperCase()==='EXPLICIT_HISTORICAL'?'EXPLICIT_HISTORICAL':'DEFAULT'}
  function researchPoolRequestDate(dateMode,formalRequestedDate,dailyResult,marketClock){
    var datePattern=/^\d{4}-\d{2}-\d{2}$/,formalDate=String(formalRequestedDate||'').slice(0,10);if(normalizedRequestedDateMode(dateMode)==='EXPLICIT_HISTORICAL')return datePattern.test(formalDate)?formalDate:'';
    var daily=dailyResult||{},clock=marketClock||{},closedDate=String(daily.authoritative_closed_trade_date||clock.authoritative_closed_trade_date||'').slice(0,10);return datePattern.test(closedDate)?closedDate:(datePattern.test(formalDate)?formalDate:'')
  }
  function researchPoolIsReadable(pool,requestedDate){
    pool=pool||{};var target=String(requestedDate||'').slice(0,10),datePattern=/^\d{4}-\d{2}-\d{2}$/,hashPattern=/^[0-9a-f]{64}$/i,buildPattern=/^[0-9a-f]{40}$/i,statusValue=String(pool.status||'').toUpperCase(),items=pool.items,summary=pool.summary||{},permissions=pool.permissions||{},allowed={VALIDATED_POSITIVE:true,PAPER_DISCOVERY_CANDIDATE:true,LEFT_SIDE_PREPARE:true,RESEARCH_ONLY_UNCALIBRATED:true};
    if(!datePattern.test(target)||pool.schema!=='probiga.trading-v3-research-pool.v1'||pool.pool_kind!=='RETROSPECTIVE_RESEARCH_OBSERVATION'||pool.pool_readable!==true||(statusValue!=='READY'&&statusValue!=='EMPTY'))return false;
    if(String(pool.decision_scope||'').toUpperCase()!=='RESEARCH_ONLY'||permissions.new_buy_eligible!==false||permissions.order_eligible!==false||permissions.notification_eligible!==false)return false;
    if(String(pool.trade_date||'').slice(0,10)!==target||String(pool.data_date||'').slice(0,10)!==target||String(pool.historical_fact_cutoff_at||'').slice(0,10)!==target)return false;
    if(!hashPattern.test(String(pool.artifact_sha256||''))||!hashPattern.test(String(pool.payload_sha256||''))||!buildPattern.test(String(pool.publisher_build_sha||'')))return false;
    var cutoffAt=Date.parse(String(pool.historical_fact_cutoff_at||'').replace(' ','T')),knownAt=Date.parse(String(pool.research_known_at||'').replace(' ','T'));if(!Number.isFinite(cutoffAt)||!Number.isFinite(knownAt)||knownAt<cutoffAt)return false;
    var stockCount=summary.observation_stock_count,matchingCount=summary.matching_forecast_count,totalCount=summary.total_forecast_count,excludedCount=summary.excluded_forecast_count,statusCounts=summary.status_forecast_counts;
    if(!Array.isArray(items)||!Number.isInteger(stockCount)||stockCount!==items.length||!Number.isInteger(matchingCount)||matchingCount<stockCount||!Number.isInteger(totalCount)||totalCount<matchingCount||!Number.isInteger(excludedCount)||excludedCount!==totalCount-matchingCount||!statusCounts||typeof statusCounts!=='object'||Array.isArray(statusCounts))return false;
    var statusKeys=Object.keys(statusCounts),allStatusTotal=statusKeys.reduce(function(total,key){var value=statusCounts[key];return total+(key&&Number.isInteger(value)&&value>=0?value:NaN)},0),matchingStatusTotal=statusKeys.reduce(function(total,key){return total+(allowed[key]?Number(statusCounts[key]||0):0)},0);if(!Number.isFinite(allStatusTotal)||allStatusTotal!==totalCount||matchingStatusTotal!==matchingCount)return false;
    var seen={};for(var i=0;i<items.length;i+=1){var item=items[i]||{},code=String(item.stock_code||''),statuses=item.statuses,strategies=item.strategy_keys,forbidden=['action_plan','buy_range','sell_range','entry_range','protective_stop','stop_loss','target_price'];if(!/^\d{6}$/.test(code)||seen[code]||!Number.isInteger(item.rank_no)||item.rank_no<1||!Array.isArray(statuses)||!statuses.length||statuses.some(function(value){return !allowed[String(value||'')]} )||!Array.isArray(strategies)||!strategies.length||!Array.isArray(item.reasons))return false;seen[code]=true;if(String(item.decision_scope||'').toUpperCase()!=='RESEARCH_ONLY'||String(item.display_action||'').toUpperCase()!=='WATCH'||String(item.actionability||'').toUpperCase()!=='RESEARCH_ONLY'||item.new_buy_eligible!==false||item.order_eligible!==false)return false;if(String(item.data_date||'').slice(0,10)!==target||String(item.research_known_at||'')!==String(pool.research_known_at||'')||String(item.publisher_build_sha||'')!==String(pool.publisher_build_sha||'')||String(item.artifact_sha256||'')!==String(pool.artifact_sha256||''))return false;if(forbidden.some(function(key){return Object.prototype.hasOwnProperty.call(item,key)}))return false;}
    return (statusValue==='READY'&&items.length>0)||(statusValue==='EMPTY'&&items.length===0&&matchingCount===0)
  }
  function shouldDisplayResearchPool(formalCurrent,pool,requestedDate,formalRequestedDate){var researchDate=String(requestedDate||'').slice(0,10),formalDate=String(formalRequestedDate||'').slice(0,10),workflow=pool&&pool.workflow||{},workflowDate=String(workflow.target_trade_date||'').slice(0,10),workflowVisible=workflowDate===researchDate&&['WAITING_FOR_DATA','CALCULATING','FAILED'].indexOf(String(workflow.status||'').toUpperCase())>=0;return(researchPoolIsReadable(pool,researchDate)||workflowVisible)&&(formalCurrent!==true||researchDate!==formalDate)}
  function researchStatusText(value){var labels={VALIDATED_POSITIVE:'历史规则命中',PAPER_DISCOVERY_CANDIDATE:'模拟试错候选',LEFT_SIDE_PREPARE:'左侧准备观察',RESEARCH_ONLY_UNCALIBRATED:'未校准研究观察'};return labels[String(value||'')]||String(value||'—')}
  function shortTradeDate(value){var parts=String(value||'').slice(0,10).split('-');return parts.length===3?Number(parts[1])+'/'+Number(parts[2]):String(value||'—')}
  function dailyDeliveryFormalTruth(payload,requestedDate,pool){
    payload=payload||{};pool=pool||{};var data=payload.data||{},session=data.session||{},receipt=data.receipt||{},target=String(requestedDate||session.trade_date||payload.trade_date||'').slice(0,10),status=String(session.status||payload.status||'').toUpperCase(),receiptStatus=String(receipt.status||'').toUpperCase(),receiptDate=String(receipt.trade_date||'').slice(0,10),poolRun=String(pool.run_uid||'').toLowerCase(),receiptRun=String(receipt.governance_run_uid||'').toLowerCase(),poolRoot=String(pool.canonical_result_hash||'').toLowerCase(),receiptRoot=String((receipt.strategy_pool||{}).root||'').toLowerCase();
    function blocked(reason,code){return {ready:false,status:status||'NOT_AVAILABLE',reason:reason,reasonCode:code||'DAILY_DELIVERY_BLOCKED'}}
    if(status!=='PASS'&&status!=='DEGRADED')return blocked(receipt.error_detail||('每日交付状态为 '+(status||'NOT_AVAILABLE')+'，不能把任务成功当作票池交付'),'DAILY_DELIVERY_'+(status||'NOT_AVAILABLE'));
    if(receiptStatus!==status||receipt.schema!=='probiga.daily-delivery.v1')return blocked('每日交付回执结构或状态不一致','DAILY_DELIVERY_RECEIPT_INVALID');
    if(!/^\d{4}-\d{2}-\d{2}$/.test(target)||receiptDate!==target)return blocked('每日交付回执日期与请求日不一致','DAILY_DELIVERY_DATE_MISMATCH');
    if(poolRun&&(receiptRun!==poolRun||!/^[0-9a-f]{64}$/.test(poolRoot)||receiptRoot!==poolRoot))return blocked('每日交付回执与当前策略池的运行身份或根哈希不一致','DAILY_DELIVERY_POOL_IDENTITY_MISMATCH');
    if(receipt.real_order_authority!==false||receipt.automatic_real_order_submission!==false)return blocked('每日交付回执没有证明真实交易权限关闭','DAILY_DELIVERY_ORDER_AUTHORITY_INVALID');
    return {ready:true,status:status,reason:status==='DEGRADED'?'核心票池准确，增强项降级已在回执中披露':'每日票池已通过最终交付回执',reasonCode:'DAILY_DELIVERY_'+status}
  }
  ...
  function requestedFrameDateMode(){
    var value='';
    try{value=String((window.frameElement&&window.frameElement.dataset.dateMode)||'')}catch(ignore){}
    return normalizedRequestedDateMode(value);
  }
  ...
  function activateView(view,options){
    ...
    if(options.userInitiated){
      if(!parentMessage('probiga-trading-v3-navigate',{view:view,requested_date:state.requestedDate,date_mode:state.requestedDateMode,filters:state.filters})){
  ...
  function runRequestQueue(items,maxConcurrent){
    if(!items.length)return Promise.resolve([]);
    var results=new Array(items.length),cursor=0,workerCount=Math.max(1,Math.min(Number(maxConcurrent||1),items.length));
    function worker(){
      var index=cursor++;if(index>=items.length)return Promise.resolve();
      var item=items[index],promise=Promise.resolve().then(item[1]);
      return requestResult(item[0],promise,item[2]).then(function(result){results[index]=result}).then(worker);
    }
    var workers=[];for(var i=0;i<workerCount;i+=1)workers.push(worker());
    return Promise.all(workers).then(function(){return results});
  }
  function applyRequestResults(results){
    (results||[]).forEach(function(result){
      delete state.pendingKeys[result.key];
      if(result.ok){state[result.key]=result.value;state.loadedKeys[result.key]=true;delete state.errors[result.key];delete state.staleKeys[result.key];return}
  ...
  function loadFormalResearch(loadSeq){
    state.pendingKeys.strategyVersions=true;state.pendingKeys.backtestHistory=true;renderFormalResearch();
    return Promise.all([
      requestResult('strategyVersions',api2('/strategies'),[]),
      requestResult('backtestHistory',api2('/research/backtests?limit=30'),[])
    ]).then(function(results){
      if(loadSeq!=null&&loadSeq!==state.loadSeq)return;
      results.forEach(function(result){
        delete state.pendingKeys[result.key];
        if(result.ok){
          if(result.key==='strategyVersions')state.strategyVersions=Array.isArray(result.value)?result.value:[];
          if(result.key==='backtestHistory')state.researchOperations={backtests:Array.isArray(result.value)?result.value:[]};
          delete state.errors[result.key];
        }else{
          state.errors[result.key]=result.error;
          if(result.key==='strategyVersions')state.strategyVersions=[];
          if(result.key==='backtestHistory')state.researchOperations={backtests:[]};
        }
      });
      renderFormalResearch();notifyParentResize();
    })
  }
  function load(){
    var loadSeq=++state.loadSeq;
    state.candidateRefreshSeq+=1;state.candidateRefreshPending=false;
    state.loading=true;state.truthLoading=true;state.errors={};state.staleKeys={};state.pendingKeys={};
    state.researchStockPool={};state.researchStockPoolRequestDate='';state.researchCandidatePage=1;delete state.loadedKeys.researchStockPool;
    el('updatedAt').textContent='读取中';
    el('truthContext').setAttribute('aria-busy','true');
    var view=state.activeView||requestedView();
    if(view==='validation')loadFormalResearch(loadSeq);
    ...
    var dailyResultPath='/daily-result'+(state.requestedDate?'?'+dateParam():''),bootstrapRequest=[
      ...
      ['readiness',function(){return api3('/readiness',30000)},{}],
      ['account',function(){return api2('/accounts/paper-main-v2')},{}],
      ['paperLedger',function(){return api3('/paper-ledger?account_id=paper-main-v2&limit=200')},{}],
      ['dailyDelivery',function(){return fetchJson('/api/scheduler/daily-delivery'+(state.requestedDate?'?'+dateParam():''))},{}],
      ['tasks',function(){return fetchJson('/api/scheduler/tasks')},[]]
    ];
    function add(key,factory,fallback){requests.push([key,factory,fallback])}
    if(view==='overview'){add('forecasts',function(){return api3(withDate('/forecasts/latest?limit=200'))},[]);add('oversold',function(){return api3(withDate('/forecasts/latest?strategy_key=oversold_reversal&limit=200'))},[]);add('targets',function(){return api3(withDate('/portfolio/latest'))},[]);add('watchlistStrategy',function(){return holdingStrategyPayload(fetchJson('/api/portfolio/holding-strategy'+(state.requestedDate?'?'+dateParam():'')))},{})}
    if(view==='hypotheses')add('hypotheses',function(){return api3(withDate('/hypotheses/latest?limit=300'))},[]);
    if(view==='candidates'){
      add('auctionGate',function(){
        var executionDate=String((state.dailyResult||{}).execution_session_date||(state.context||{}).execution_session_date||(state.stockPool||{}).execution_session_date||'').slice(0,10);
        if(!/^\d{4}-\d{2}-\d{2}$/.test(executionDate))return Promise.reject(new Error('缺少策略池对应的 execution_session_date，禁止按决策日读取集合竞价'));
        return api3('/premarket/auction-gate?execution_session_date='+encodeURIComponent(executionDate));
      },{});
    }
    if(view==='intraday')add('intraday',function(){return api2('/accounts/paper-main-v2/intraday?limit=200')},{});
    if(view==='portfolio')add('targets',function(){return api3(withDate('/portfolio/latest'))},[]);
    if(view==='positions'){add('positions',function(){return api2('/accounts/paper-main-v2/positions')},[]);add('targets',function(){return api3(withDate('/portfolio/latest'))},[]);add('watchlistStrategy',function(){return holdingStrategyPayload(fetchJson('/api/portfolio/holding-strategy'+(state.requestedDate?'?'+dateParam():'')))},{})}
    if(view==='validation'){add('validation',function(){return api3('/validation/latest')},null);add('dataEvidence',function(){return api2('/system/data-evidence')},{});add('researchGovernance',governanceApi,{})}
    if(view==='missed'){add('recall',function(){return api3('/opportunity-recall/latest')},null);add('learning',function(){return api3('/learning/oversold_reversal')},{});add('researchGovernance',governanceApi,{})}
    if(view==='evidence'){add('dataEvidence',function(){return api2('/system/data-evidence')},{});add('researchGovernance',governanceApi,{})}
    bootstrapRequest.concat(requests).forEach(function(item){state.pendingKeys[item[0]]=true});
    return runRequestQueue(bootstrapRequest,1).then(function(results){
      if(loadSeq!==state.loadSeq)return;
      applyRequestResults(results);
      var bootstrapResult=results[0]||{};
      if(!bootstrapResult.ok){
        state.errors.context=bootstrapResult.error||'API_DOWN：每日原子结果接口不可用';
        state.errors.overview=state.errors.context;
        throw new Error(state.errors.context);
      }
      var daily=bootstrapResult.value||{};
      if(!daily.context||!daily.overview||!daily.stock_pool){
        state.errors.context=state.errors.dailyResult||'API_DOWN：每日原子结果接口不可用';
        state.errors.overview=state.errors.context;
        throw new Error(state.errors.context);
      }
      state.context=daily.context;state.overview=daily.overview;state.stockPool=daily.stock_pool;
      state.loadedKeys.context=true;state.loadedKeys.overview=true;state.loadedKeys.stockPool=true;
      delete state.pendingKeys.context;delete state.pendingKeys.overview;delete state.pendingKeys.stockPool;
      state.truthLoading=false;
      el('updatedAt').textContent='同批次结果已加载 · 补充数据后台读取中';renderAll();activateView(view);notifyParentResize();
      if(view==='candidates'&&!stockPoolIsReadable(state.stockPool))add('stockPool',function(){return stockPoolWithHistoricalFallback(state.requestedDate)},{});
      if(view==='candidates'){
        var formalRequestDate=String(state.requestedDate||(daily.context||{}).requested_date||(daily.context||{}).requested_trade_date||(daily.stock_pool||{}).requested_trade_date||(daily.context||{}).decision_session_date||(daily.stock_pool||{}).trade_date||'').slice(0,10),researchRequestDate=researchPoolRequestDate(state.requestedDateMode,formalRequestDate,daily,state.marketClock);
        if(/^\d{4}-\d{2}-\d{2}$/.test(researchRequestDate)){
          state.researchStockPoolRequestDate=researchRequestDate;state.pendingKeys.researchStockPool=true;
          add('researchStockPool',function(){var requestDate=researchRequestDate,requestSeq=loadSeq;return api3('/research/stock-pool?trade_date='+encodeURIComponent(requestDate),60000).then(function(payload){if(requestSeq!==state.loadSeq||requestDate!==state.researchStockPoolRequestDate)return {};return unwrap(payload)||{}})},{});
        }
      }
      return runRequestQueue(requests,3);
    }).then(function(results){
      if(loadSeq!==state.loadSeq)return;
      applyRequestResults(results);
    ...
    }).catch(function(err){if(loadSeq!==state.loadSeq)return;state.loading=false;state.errors.load=errorText(err);el('updatedAt').textContent='API_DOWN';renderTruthContext();el('heroTitle').textContent='API_DOWN：每日结果接口不可用';el('heroReason').textContent=errorText(err);el('hero').classList.add('blocked');el('truthContext').setAttribute('aria-busy','false');notifyParentResize()})
  }
  function renderAll(){renderTruthContext();renderChrome();renderActions();renderOverview();renderWatchlistStrategy();renderThemeAudit();renderHypotheses();renderCandidates();renderIntraday();renderPortfolio();renderPositions();renderOrders();renderFormalResearch();renderValidation();renderRecall();renderLearning();renderEvidence();renderLineage();renderBatchDiff();renderAdvisoryResearch();renderHorizonResearch();renderCounterfactualResearch();renderShadowGovernance()}
  function truthState(){
    var daily=state.dailyResult||{},ctx=state.context||{},run=(state.overview||{}).run||{},delivery=String(daily.delivery_status||'').toUpperCase();
    if(state.truthLoading)return {code:'LOADING',reason:'正在核对决策批次、证据时点和执行权限；完成前不展示空白结论。'};
    ...
    if(delivery==='DEGRADED')return {code:'BLOCKED',label:'DEGRADED',reason:'规范批次已读取，但两端调度器或发布身份未同时通过：'+String(daily.reason_code||'SCHEDULER_NOT_HEALTHY')+'。'};
    if(delivery==='COMPLETED'&&(!daily.acceptance||daily.acceptance.accepted!==true))return {code:'UNAVAILABLE',reason:'每日原子结果声称 COMPLETED，但验收回执未通过；系统按 fail-closed 处理。'};
    if(ctx.run_uid&&run.run_uid&&String(ctx.run_uid)!==String(run.run_uid))return {code:'UNAVAILABLE',reason:'统一上下文与页面决策投影的批次不一致；当前结论不可用。'};
    ...
    if(loading)return {code:'LOADING',reason:ctx.data_blocked_reason||ctx.analysis_stage||'决策批次仍在生成或归集证据；当前内容不是最终结论，禁止发起新动作。'};
    ...
    if(/BLOCK|REJECT|HALT|PAUSE/.test(dataStatus)||/BLOCK|REJECT|HALT|PAUSE/.test(decision))return {code:'BLOCKED',label:dataStatus==='DATA_BLOCKED'?'DATA_BLOCKED':'BLOCKED',reason:ctx.data_blocked_reason||'数据或决策门禁未通过，不允许新增模拟订单。'};
    if(ctx.historical_read_only===true)return {code:'STALE',reason:'当前为历史证据复核；自选持仓按当前登记范围回看证据，模拟账户仍是当前快照，不能还原历史账户。'};
    ...
    if(state.pendingKeys.overview)return {code:'LOADING',reason:'统一上下文已读取，正在核对同批次目标投影；完成前不解释为空池。'};
  ...
  function decisionStateLabel(truth){return {READY:'研究结果已验证',EMPTY:'已验证无目标',LOADING:'正在核对',STALE:'历史或过期证据',BLOCKED:'条件未通过',UNAVAILABLE:'证据不可用'}[truth.code]||'等待核验'}
  ...
  function renderTruthContext(){
    ...
    var requested=ctx.requested_date||state.requestedDate||'—',sessionDate=ctx.decision_date||ctx.decision_session_date||ctx.requested_date||run.decision_date||run.decision_session_date||'—',executionDate=ctx.execution_session_date||run.execution_session_date||'—',dataDate=ctx.data_date||run.trade_date||'—',expectedDataDate=ctx.expected_data_date||dataDate;
    el('truthContext').dataset.state=truth.code;el('truthState').textContent=decisionStateLabel(truth);el('truthStateReason').textContent=truth.reason;
    el('ctxRequestedDate').textContent=requested;el('ctxDecisionSessionDate').textContent=sessionDate;el('ctxExecutionSessionDate').textContent=executionDate;el('ctxDataDate').textContent=dataDate;el('ctxExpectedDataDate').textContent=expectedDataDate;el('ctxRunUid').textContent=ctx.run_uid||ctx.upstream_run_uid||run.run_uid||'—';el('ctxDecisionAt').textContent=ctx.decision_at||run.decision_at||'—';el('ctxEvidenceAsOf').textContent=ctx.evidence_as_of||run.evidence_as_of||run.decision_at||'—';el('ctxValidUntil').textContent=ctx.valid_until||run.valid_until||'—';
    ...
    var paperLabel=paperAllowed?'模拟可入队':truth.code==='LOADING'?'等待批次':truth.code!=='READY'?'不可入队':scope==='RESEARCH_ONLY'?'仅供研究':state.errors.readiness?'执行权限不可用':'执行复验阻断';
  ...
  function renderChrome(){
    ...
    var displayPnl=firstNumber([ledgerSummary.total_unrealized_pnl]);
    ...
    el('riskCap').textContent='风险仓位上限 '+fractionPct(firstNumber([run.risk_asset_cap,regime.risk_asset_cap]),1);
    el('equity').textContent=money(displayEquity);
    el('cash').textContent=money(displayCash);
    el('equitySource').textContent=scopeText;
    el('cashSource').textContent='待买资金 '+money(ledgerSummary.legacy_pending_buy_amount)+'；已实现盈亏 '+money(ledgerSummary.legacy_realized_pnl);
    el('unrealizedPnl').textContent=(displayPnl>0?'+':'')+money(displayPnl);
    el('unrealizedPnl').className=pnlClass(displayPnl);
    el('marketValue').textContent='最新持仓市值 '+money(ledgerSummary.current_market_value);
    el('positionCount').textContent=String(positionCount)+' 只'+(positionLots!==positionCount?' / '+String(positionLots)+' 笔':'');
    el('validated').textContent=num(run.validated_count,0);
    ...
    var truth=truthState(),holdingPending=state.pendingKeys.watchlistStrategy===true,holdingSummary=(state.watchlistStrategy||{}).summary||{},holdingRows=(state.watchlistStrategy||{}).rows||[],urgentHolding=holdingRows.find(function(x){return x.exit_intent==='SELL'||x.exit_intent==='REDUCE'});
    var holdingError=!!state.errors.watchlistStrategy,holdingHistorical=(state.watchlistStrategy||{}).historical_read_only===true||(state.context||{}).historical_read_only===true,holdingUnknown=holdingPending||holdingError||!state.loadedKeys.watchlistStrategy;
    ...
    var waitCount=Number(holdingSummary.wait_data_count||0),unknownAction=holdingUnknown||waitCount>0;
    el('actualHoldingCount').textContent=holdingUnknown?'—':String(holdingCount)+' 只';
    el('urgentActionCount').textContent=holdingUnknown||holdingHistorical?'—':String(urgentCount)+' 只';
    el('urgentActionSummary').textContent=holdingError?'持仓策略不可用':holdingUnknown?'正在核对持仓退出红线':holdingHistorical?'历史证据复核，不是今日动作':sellCount?'退出 '+String(sellCount)+' · 减仓 '+String(reduceCount):reduceCount?'减仓 '+String(reduceCount):waitCount?String(waitCount)+' 只等待可靠证据':holdingCount?'当前无强制卖出':'尚未登记真实持仓';
    el('allowedBuyCount').textContent=holdingHistorical?'历史只读':unknownAction?'待核验':String(urgentCount||truth.code!=='READY'?0:actionableTargets.length)+' 只';
    el('intradayRedlineCount').textContent=holdingUnknown||holdingHistorical?'—':String(redlineCount)+' 条';
    if(holdingHistorical){el('todayFirstAction').textContent='复核历史证据，不形成今天的动作';el('todayFirstActionReason').textContent='按当前登记持仓核对所选日期的证据，不还原当日持仓；模拟账户仍为当前快照。'}
    else if(holdingError){el('todayFirstAction').textContent='冻结新增风险，先恢复持仓数据';el('todayFirstActionReason').textContent='持仓策略读取失败，不能把缺失数据解释为继续持有。'}
    else if(holdingUnknown){el('todayFirstAction').textContent='正在核对持仓风险';el('todayFirstActionReason').textContent='持仓策略尚未读取完成，暂不把缺失结果解释为无持仓或继续持有。'}
    ...
    else if(waitCount){el('todayFirstAction').textContent='先核对 '+String(waitCount)+' 只持仓缺失的证据';el('todayFirstActionReason').textContent='缺少有效价格或决策依据时，无法判断继续持有或新增买入；先复核已有风险。'}
    else if(holdingCount){el('todayFirstAction').textContent='检查 '+String(holdingCount)+' 只持仓红线';el('todayFirstActionReason').textContent='依据已验证的持仓策略复核保护位与可卖数量；价格或趋势变化后重新判断。'}
    else{el('todayFirstAction').textContent='先确认自选股持仓是否已登记';el('todayFirstActionReason').textContent='成本价、股数和买入日期完整后，系统才会连续给出后续操作。'}
    if(holdingHistorical){el('heroTitle').textContent='历史证据复核 · '+String(state.requestedDate||(state.watchlistStrategy||{}).trade_date||'').slice(0,10);el('heroReason').textContent='所选日期的判断仅供复盘，不能作为当前买卖指令。下方持仓按当前登记范围核对历史证据。';el('hero').classList.add('blocked')}
    else if(holdingError){el('heroTitle').textContent='持仓风险尚无法核验';el('heroReason').textContent='持仓策略读取失败，暂不形成新增买入或继续持有结论。';el('hero').classList.add('blocked')}
    else if(!holdingUnknown&&Number(holdingSummary.sell_count||0)>0){el('heroTitle').textContent='自选股持仓有 '+String(holdingSummary.sell_count)+' 只需要优先复核退出';el('heroReason').textContent=urgentHolding?(urgentHolding.short_name||urgentHolding.stock_code)+'：'+(urgentHolding.reason||urgentHolding.action):'持仓退出信号优先于新增买入候选；请先处理风险。';el('hero').classList.add('blocked')}
    ...
    else if(truth.code==='BLOCKED'){el('heroTitle').textContent=truth.label==='DATA_BLOCKED'?'数据阻断（DATA_BLOCKED），本批次未形成策略池':'决策门禁已阻断';el('heroReason').textContent=truth.reason;el('hero').classList.add('blocked')}
    else if(holdingUnknown||waitCount){el('heroTitle').textContent='决策已读取，正在核对持仓风险';el('heroReason').textContent='持仓证据不完整，暂不形成新增买入或继续持有结论。';el('hero').classList.add('blocked')}
    else if(scope==='RESEARCH_ONLY'&&targets.length){el('heroTitle').textContent='已形成研究目标，但不拥有订单权限';el('heroReason').textContent='当前仅形成研究排序，还未取得模拟买入资格。';el('hero').classList.add('blocked')}
    else if(truth.code==='READY'&&ready===false){el('heroTitle').textContent='研究决策有效，模拟执行门禁阻断';el('heroReason').textContent='研究结果已验证；账户、价格与风险约束尚未全部通过，暂不能进入模拟买单。';el('hero').classList.add('blocked')}
  ...
  function renderOverview(){
    ...
    el('decisionStatus').textContent=decisionStateLabel(truth);el('decisionStatus').title=portfolio.status?'source portfolio.status = '+String(portfolio.status):'';el('decisionStatus').className='badge '+(truth.code==='READY'||truth.code==='EMPTY'?'safe':truth.code==='UNAVAILABLE'?'danger':'warning');
    el('decisionFacts').innerHTML=[
      fact('决策日期',run.trade_date||'—'),fact('市场状态',run.dominant_regime||regime.dominant_state||'—'),fact('目标风险资产',fractionPct(portfolio.target_risk_asset_weight,1)),fact('目标现金',money(portfolio.target_cash)),fact('组合预期收益',uncalibratedOnly?'未校准（仅模拟试错）':pct(portfolio.expected_portfolio_return_pct)),fact('最坏开放风险',money(portfolio.worst_case_loss_cny))
    ].join('');
    ...
    el('overviewTargets').innerHTML=targets.length?targets.map(function(x){var calibrated=hasCalibratedExpectation(x);return '<tr><td>'+security(x.stock_code,x.stock_name||x.short_name)+'</td><td>'+fractionPct(x.target_weight,1)+'</td><td>'+num(x.target_quantity,0)+'</td><td class="'+(calibrated?'safe-text':'warning-text')+'">'+(calibrated?pct(x.expected_return_net_pct):'未校准')+'</td><td>'+(calibrated?pct(x.conservative_return_pct):'未校准')+'</td><td>'+pct(x.estimated_roundtrip_cost_pct)+'</td><td class="reason">'+esc(x.reason||'—')+'</td></tr>'}).join(''):decisionEmptyFor('targets',7,'已验证批次没有研究目标；研究结果不会直接形成订单');
  }
  function renderWatchlistStrategy(){
    var payload=state.watchlistStrategy||{},rows=Array.isArray(payload.rows)?payload.rows:[],summary=payload.summary||{},error=state.errors.watchlistStrategy,pending=state.pendingKeys.watchlistStrategy===true,historical=payload.historical_read_only===true;
  ...
  function renderHypotheses(){
    if(state.errors.hypotheses||state.pendingKeys.hypotheses){var failed=!!state.errors.hypotheses;el('marketHypothesisState').textContent=failed?'证据不可用':'读取中';el('marketHypothesisState').className='badge '+(failed?'danger':'warning');el('marketHypothesisFacts').innerHTML=fact('当前状态',failed?'读取失败，不能把缺失证据解释为没有风险或没有机会':'正在核对所选日期的证据');el('hypothesisRows').innerHTML=empty(10,failed?'连续跟踪读取失败：'+state.errors.hypotheses:'正在读取所选日期的交易假设');return}
    ...
    el('marketHypothesisFacts').innerHTML=market.hypothesis_id?[
      ...
      fact('判断量化',hypothesisEstimate(market.probability,market.probability_kind)),
      fact('概率性质',probabilityName(market.probability_kind)),
      fact('建议动作',actionName(market.proposed_action)),
      fact('最大风险仓位',fractionPct(market.max_position_weight,1)),
  ...
  function showHypothesis(hypothesisId){
    ...
    api3('/hypotheses/'+encodeURIComponent(hypothesisId)+'/timeline?limit=500').then(function(payload){
      ...
      var blocks=[
        '<p class="note">完整后续验证：以下包含该假设建立后的证据，不代表所选日期当时已知的信息。请按每条证据的发生时间复核。</p>',
      ...
      blocks.push(events.length?'<ol class="evidence-timeline">'+events.map(function(x){return '<li><time>'+esc(x.observed_at||'—')+'</time><div><strong>'+esc(hypothesisName(x.state_before))+' → '+esc(hypothesisName(x.state_after))+'</strong><span>'+esc(hypothesisEstimate(x.probability_before,x.probability_kind||h.probability_kind))+' → '+esc(hypothesisEstimate(x.probability_after,x.probability_kind||h.probability_kind))+'</span><p>'+esc(x.summary||'—')+'</p></div></li>'}).join('')+'</ol>':'<p class="note">当前只有日级初始假设，盘中证据尚未写入。</p>');
      el('hypothesisDetail').innerHTML=blocks.join('');
      el('hypothesisDetailPanel').scrollIntoView({behavior:'smooth',block:'start'});
    }).catch(function(err){if(requestSeq!==state.hypothesisDetailRequestSeq||loadSeq!==state.loadSeq)return;el('hypothesisDetail').innerHTML='<p class="note">证据读取失败：'+esc(err.message)+'</p>'})
  }
  function reloadHypotheses(){
    ...
    var requestSeq=(state.hypothesisRequestSeq||0)+1,loadSeq=state.loadSeq;state.hypothesisRequestSeq=requestSeq;state.hypothesisDetailRequestSeq=(state.hypothesisDetailRequestSeq||0)+1;state.hypotheses=[];state.hypothesisDetail=null;state.pendingKeys.hypotheses=true;delete state.errors.hypotheses;el('hypothesisDetail').innerHTML='';el('hypothesisDetailTitle').textContent='点击“看证据”查看完整后续验证';renderHypotheses();
    syncFilters();api3('/hypotheses/latest?'+params.join('&')).then(function(v){if(requestSeq!==state.hypothesisRequestSeq||loadSeq!==state.loadSeq)return;delete state.errors.hypotheses;delete state.pendingKeys.hypotheses;state.hypotheses=unwrap(v)||[];renderHypotheses()}).catch(function(err){if(requestSeq!==state.hypothesisRequestSeq||loadSeq!==state.loadSeq)return;delete state.pendingKeys.hypotheses;state.errors.hypotheses=errorText(err);renderHypotheses();renderTruthContext()})
  }
  function renderResearchObservationPool(formalCurrent,requestedDate,formalRequestedDate){
    var box=el('candidateReconstructionPool'),pool=state.researchStockPool||{},target=String(requestedDate||state.researchStockPoolRequestDate||'').slice(0,10),formalTarget=String(formalRequestedDate||'').slice(0,10),visible=!state.errors.researchStockPool&&shouldDisplayResearchPool(formalCurrent,pool,target,formalTarget);if(!box)return {visible:false,filteredCount:0,totalCount:0};box.hidden=!visible;
    if(!visible){el('candidateReconstructionCards').innerHTML='';el('candidateReconstructionRows').innerHTML='';el('candidateReconstructionPager').hidden=true;return {visible:false,filteredCount:0,totalCount:0}}
    if(!researchPoolIsReadable(pool,target)){var workflow=pool.workflow||{},workflowStatus=String(workflow.status||'WAITING_FOR_DATA').toUpperCase(),workflowText=workflowStatus==='CALCULATING'?'数据已齐，票池正在计算':workflowStatus==='FAILED'?'票池计算失败，系统将继续重试':'正在补齐票池所需数据',reason=String(workflow.reason_code||'RESEARCH_INPUTS_NOT_READY'),retry=workflow.next_retry_at?'，下次重试 '+displayDateTime(workflow.next_retry_at):'';el('candidateReconstructionTitle').textContent=shortTradeDate(target)+' 研究观察票池';el('candidateReconstructionAuthority').textContent=workflowStatus==='FAILED'?'计算失败':'自动恢复中';el('candidateReconstructionMeta').textContent=workflowText+'。目标交易日 '+target+'，原因 '+reason+retry+'。';el('candidateReconstructionCards').innerHTML='<p class="empty">'+esc(workflowText)+'。</p>';el('candidateReconstructionRows').innerHTML=empty(6,workflowText+'。');el('candidateReconstructionPager').hidden=true;return {visible:true,filteredCount:0,totalCount:0}}
    var items=pool.items||[],sf=el('strategyFilter').value,st=el('statusFilter').value,q=el('candidateSearch').value.trim().toLowerCase(),rows=items.filter(function(item){var haystack=[item.stock_code,item.stock_name,(item.strategy_keys||[]).map(strategy).join(' '),(item.statuses||[]).map(researchStatusText).join(' '),(item.reasons||[]).join(' ')].join(' ').toLowerCase();return(!sf||(item.strategy_keys||[]).indexOf(sf)>=0)&&(!st||st==='RESEARCH_ONLY')&&(!q||haystack.indexOf(q)>=0)});
    var pageSize=40,pageCount=Math.max(1,Math.ceil(rows.length/pageSize));state.researchCandidatePage=Math.max(1,Math.min(pageCount,Number(state.researchCandidatePage||1)));var start=(state.researchCandidatePage-1)*pageSize,pageRows=rows.slice(start,start+pageSize);
    function observationStatus(item){return (item.statuses||[]).map(researchStatusText).join(' / ')||'研究观察'}
    function reasons(item){return (item.reasons||[]).join('；')||'当前只保留观察身份，等待后续前向验证'}
    function card(item){return '<article class="candidate-card"><div class="candidate-card-head"><span class="candidate-rank">#'+esc(num(item.rank_no,0))+'</span>'+security(item.stock_code,item.stock_name)+'<span class="badge warning">仅供观察</span></div><div class="candidate-card-route"><strong>'+esc((item.strategy_keys||[]).map(strategy).join(' / ')||'—')+'</strong><span>'+esc(observationStatus(item))+'</span></div><div class="candidate-card-metrics"><div><span>综合分</span><strong>'+num(item.raw_score,3)+'</strong></div><div><span>数据日</span><strong>'+esc(item.data_date||target)+'</strong></div><div><span>结果性质</span><strong>日终研究</strong></div><div><span>当前动作</span><strong>继续观察</strong></div></div><p class="candidate-card-reason"><b>观察原因：</b>'+esc(reasons(item))+'</p></article>'}
    el('candidateReconstructionTitle').textContent=shortTradeDate(target)+' 研究观察票池';el('candidateReconstructionAuthority').textContent='仅供观察';el('candidateReconstructionMeta').textContent='已筛出符合观察条件的股票，尚未满足正式买入条件。数据日 '+target+'，实际生成 '+displayDateTime(pool.generated_at||pool.published_at||pool.research_known_at)+'，共 '+Number((pool.summary||{}).observation_stock_count||0)+' 只；页面只展示观察原因，不会触发买卖或下单。';
    var emptyText=String(pool.status||'').toUpperCase()==='EMPTY'?'该日已完成筛选，没有符合观察条件的股票。':'当前筛选条件下没有研究观察股票。';el('candidateReconstructionCards').innerHTML=pageRows.length?pageRows.map(card).join(''):'<p class="empty">'+esc(emptyText)+'</p>';el('candidateReconstructionRows').innerHTML=pageRows.length?pageRows.map(function(item){return '<tr><td>'+num(item.rank_no,0)+'</td><td>'+security(item.stock_code,item.stock_name)+'</td><td>'+esc((item.strategy_keys||[]).map(strategy).join(' / ')||'—')+'</td><td>'+esc(observationStatus(item))+' · 仅供观察</td><td>'+num(item.raw_score,3)+'</td><td class="reason">'+esc(reasons(item))+'</td></tr>'}).join(''):empty(6,emptyText);
    el('candidateReconstructionPageStatus').textContent='第 '+state.researchCandidatePage+' / '+pageCount+' 页';el('candidateReconstructionPrev').disabled=state.researchCandidatePage<=1;el('candidateReconstructionNext').disabled=state.researchCandidatePage>=pageCount;el('candidateReconstructionPager').hidden=rows.length<=pageSize;return {visible:true,filteredCount:rows.length,totalCount:items.length}
  }
  function renderCandidates(){
    ...
    var formalRequestedDate=String(state.requestedDate||pool.requested_trade_date||'').slice(0,10),researchRequestedDate=String(state.researchStockPoolRequestDate||formalRequestedDate).slice(0,10),researchDisplay=renderResearchObservationPool(formalCurrent,researchRequestedDate,formalRequestedDate),researchPoolVisible=researchDisplay.visible===true;
    ...
    if(pool.exact_run_missing&&!historicalFallback)candidateEmptyText='请求日没有已完成的决策，且没有更早的可读历史策略池可供回看';
    ...
    var researchNotice=el('candidateResearchNotice');if(researchNotice){researchNotice.hidden=formalCurrent&&!researchDefaultFallback&&!researchPoolVisible;researchNotice.textContent=researchPoolVisible?'最新收盘日 '+researchRequestedDate+' 的研究观察结果已生成；这些股票尚未满足正式买入条件。':governanceDeferred?'DEFERRED_DB / 治理数据库迁移延期：策略池仅保留不可变审计行，全部为 RESEARCH_ONLY；当前不展示可执行买卖区间或止损指令，也不会创建模拟或真实订单。':!formalCurrent&&!historicalFallback?'RESEARCH_ONLY / 正式票池不可用：'+formalTruth.reason+'。旧日期、未验证或研究只读候选不会进入当前可执行展示。':researchDefaultFallback?'本批次没有“允许买入区间、等待触发或仅模拟研究”候选；当前按策略排名展示研究观察股票。它们没有可执行资格，不会创建模拟或真实订单。':''}
    var daily=pool.daily_change||{},strategyExecution=pool.strategy_execution||{},strategyRows=strategyExecution.strategies||[],dailyNotice=el('candidateDailyNotice');if(dailyNotice)dailyNotice.textContent=String(daily.status||'')==='NO_PREVIOUS_BATCH'?'每日变化 / 当前是首个可验证比较基线；在产生下一批次前，不把这些股票误称为“今日新入”。':'每日变化 / 每日重算：'+Number(daily.new_count||0)+' 只新入，'+Number(daily.retained_count||0)+' 只连续保留，'+Number(daily.upgraded_count||0)+' 只上升，'+Number(daily.downgraded_count||0)+' 只下降，'+Number(daily.removed_count||0)+' 只移出。连续保留不是静态不动，每只股票都展示当日排名或分数变化原因。';
    var auctionNotice=el('candidateAuctionNotice');if(auctionNotice){var gateSummary=gate.summary||{},gateReason=gate.reason||'',gateStatus=String(gate.status||'UNAVAILABLE'),dateIdentity='决策日 '+(poolDecisionDate||'—')+' → 执行日 '+(poolExecutionDate||'—');auctionNotice.textContent=state.errors.auctionGate?'AUCTION UNAVAILABLE / '+dateIdentity+'；竞价重评读取失败：'+state.errors.auctionGate+'；这不等于没有可买候选。':!gateAligned?'AUCTION PENDING / '+dateIdentity+'；尚无与当前策略批次和执行日严格对应的竞价结论；不会用其他日期或其他批次替代。':gateStatus==='COMPLETED'?'AUCTION '+String(gate.stage||'REVIEW')+' / '+dateIdentity+' / 截止 '+displayDateTime(gate.cutoff_at)+'：全池复核 '+Number(gateSummary.reviewed_count||0)+' 只，竞价确认 '+Number(gateSummary.buy_candidate_count||0)+' 只，等待开盘 '+Number(gateSummary.wait_count||0)+' 只，数据/成交阻断 '+Number(gateSummary.blocked_count||0)+' 只，拒绝追高或弱势 '+Number(gateSummary.rejected_count||0)+' 只。备选关系只用于比较，automatic_substitution=false。':'AUCTION '+gateStatus+' / '+dateIdentity+' / '+(gateReason||'集合竞价结论尚未形成')+'；不会提前或机械递补。'}
    ...
    el('candidateSummary').textContent=researchPoolVisible?'观察日 '+researchRequestedDate+' · 研究观察票池 '+researchDisplay.filteredCount+' / '+researchDisplay.totalCount+' 只 · 仅供观察':(formalCurrent?(deliveryTruth.status==='DEGRADED'?'DEGRADED · 核心票池已验证 · ':pool.is_as_of_fallback===true?'已验证 · 最新成功结果 · ':'已验证 · 当前正式票池 · '):governanceDeferred?'治理延期 · 只读研究 · ':historicalFallback?'历史只读 · ':researchDefaultFallback?'研究观察 · ':'')+'请求日 '+requestedDate+' · 决策日 '+decisionDate+' · 数据日 '+dataDate+' · 执行日 '+(poolExecutionDate||'—')+' · 当前显示 '+rows.length+' 条'+(formalCurrent?'':' · 阻断 '+formalTruth.reasonCode);el('candidatePageStatus').textContent='第 '+state.candidatePage+' / '+pageCount+' 页';el('candidatePrev').disabled=state.candidatePage<=1;el('candidateNext').disabled=state.candidatePage>=pageCount;el('candidatePager').hidden=rows.length<=pageSize;
  }
  function renderIntraday(){
    ...
    el('intradayHistoricalFacts').innerHTML=history&&history.observed_at?[
      fact('历史快照时间',history.observed_at),fact('身份','历史/收盘快照，仅供复盘'),fact('市场状态',history.state||'—'),fact('行情来源',sourceName(history.source_provider)),fact('有效覆盖率',fractionPct(history.coverage,1)),fact('执行权限','禁止下单')
  ...
  function formalStrategyLabel(row){
    row=row||{};var manifest=row.manifest||{};
    return manifest.display_name||manifest.name||strategy(row.strategy_id)||row.strategy_id||'未命名策略';
  }
  function formalLifecycle(value){
    var labels={DRAFT_BLOCKED:'配置阻断',DRAFT:'待研究',RESEARCH:'研究中',OOS_PASSED:'样本外已通过',SHADOW:'影子观察',PAPER_TRIAL:'模拟试运行',PAPER_ACTIVE:'模拟运行',SUSPENDED:'已暂停',RETIRED:'已淘汰'};
    return labels[String(value||'').toUpperCase()]||String(value||'未运行');
  }
  function formalAdapterSupported(row){return !!(row&&row.backtest_adapter_supported===true)}
  function formalBacktestRunnable(row){
    if(!formalAdapterSupported(row)||!row||row.formal_fee_coverage_usable!==true||row.formal_data_contract_status!=='COMPLETE')return false;
    var feeStart=String(row.earliest_fee_covered_start||''),end=String(row.latest_runnable_session||'');
    return !!feeStart&&!!end&&feeStart<=end;
  }
  function formalFeeCoverageLabel(row){
    if(!row||row.formal_fee_coverage_usable!==true)return '费率覆盖不可用';
    return String(row.earliest_fee_covered_start||'—')+' 至 '+String(row.latest_fee_covered_end||'长期有效');
  }
  function formalRunClassification(row){
    row=row||{};var runStatus=String(row.status||'').toUpperCase(),report=row.result||{},promotion=report.promotion_protocol||{},gate=String(row.gate_status||promotion.status||'BLOCK').toUpperCase(),blockers=(promotion.blockers||[]).concat(promotion.reason||[]).filter(Boolean),joined=blockers.join(' ').toUpperCase();
    if(!runStatus)return {code:'NOT_RUN',label:'未运行',kind:'warning'};
    if(runStatus==='FAILED')return {code:'FAILED',label:'运行失败',kind:'danger'};
    if(/RUNNING|PENDING|QUEUED|CREATED/.test(runStatus))return {code:'RUNNING',label:'运行中',kind:'warning'};
    if(runStatus!=='COMPLETED')return {code:'UNKNOWN',label:'状态待确认',kind:'danger'};
    if(report.adapter==='unified_screener_point_in_time_v2')return {code:'ADAPTER_RETIRED',label:'历史适配器不匹配，仅保留记录',kind:'danger'};
    if(/STATISTICAL_GATE_BLOCKED|BASELINE_COMPARISON_BLOCKED/.test(joined))return {code:'NOT_QUALIFIED',label:'表现不合格',kind:'danger'};
    if(gate==='PASS')return {code:'PASS',label:'研究门槛通过',kind:'safe'};
    if(/INSUFFICIENT|SAMPLE|TRADE_LEVEL_LEDGER|HOLDOUT_NOT_PRISTINE|EVIDENCE_REQUIRED|COLLECTING/.test(joined))return {code:'INSUFFICIENT',label:'证据不足',kind:'warning'};
    return {code:'NOT_QUALIFIED',label:'表现不合格',kind:'danger'};
  }
  function formalBacktestMetrics(row){
    var report=(row||{}).result||{},stock=report.strategy_result||{},horizons=stock.horizons||{},primary=horizons['T+5']||horizons['T+20']||horizons['T+1']||{},metrics=report.metrics||{},performance=report.performance||{},etfReturn=performance.total_return==null?null:Number(performance.total_return)*100,etfDrawdown=performance.max_drawdown==null?null:Math.abs(Number(performance.max_drawdown))*100;
    return {
      sample:firstNumber([primary.sample,metrics.completed_trade_count,report.completed_trade_count]),
      netReturn:firstNumber([primary.net_average_pct,performance.total_return_pct,metrics.net_return_pct,etfReturn]),
      maxDrawdown:firstNumber([primary.net_max_drawdown_pct,performance.maximum_drawdown_pct,performance.max_drawdown_pct,metrics.maximum_drawdown_pct,etfDrawdown]),
      finalEquity:firstNumber([primary.ending_nav_cny,metrics.final_equity_cny,report.final_equity_cny]),
      unfilled:firstNumber([(primary.execution_status_counts||{}).KNOWN_UNFILLED,report.blocked_orders]),
      unresolved:firstNumber([(primary.execution_status_counts||{}).UNRESOLVED_EXIT,metrics.unresolved_position_count,report.open_position_count])
    };
  }
  function formalBacktestIdentity(row){
    row=row||{};var verified=String(row.strategy_identity_status||'').toUpperCase()==='VERIFIED',strategyId=verified?String(row.strategy_id||''):'',strategyVersion=verified?String(row.strategy_version||''):'';
    if(!strategyId||!strategyVersion)return {verified:false,label:'策略身份不可确认'};
    return {verified:true,strategy_id:strategyId,strategy_version:strategyVersion,label:strategyId+' @ '+strategyVersion};
  }
  function formalComparisonConditions(row){
    var identity=formalBacktestIdentity(row),report=(row||{}).result||{},assumptions=report.execution_assumptions||{},values={
      start_date:row.start_date||'',end_date:row.end_date||'',random_seed:row.random_seed,
      initial_capital_cny:assumptions.initial_capital_cny,
      cost_scenario_multiplier:assumptions.cost_scenario_multiplier,
      top_per_day:report.adapter==='unified_screener_point_in_time_v2'?assumptions.top_per_day:'NOT_APPLICABLE',
      adapter:report.adapter||'',protocol_version:row.protocol_version||'',
      code_commit_sha:row.code_commit_sha||'',data_snapshot_hash:row.data_snapshot_hash||''
    },missing=Object.keys(values).filter(function(key){return values[key]==null||values[key]===''||values[key]==='0'.repeat(64)});if(!identity.verified)missing.unshift('strategy_identity');return {values:values,missing:missing,key:JSON.stringify(values)};
  }
  function renderFormalBacktestComparison(){
    var node=el('formalBacktestComparison');if(!node)return;var ids=state.compareBacktests||[];if(ids.length!==2){node.innerHTML='';return}
    var rows=((state.researchOperations||{}).backtests)||[],left=rows.find(function(x){return String(x.backtest_uid)===ids[0]}),right=rows.find(function(x){return String(x.backtest_uid)===ids[1]});if(!left||!right){node.innerHTML='';return}
    var leftConditions=formalComparisonConditions(left),rightConditions=formalComparisonConditions(right),leftLabel=formalBacktestIdentity(left).label,rightLabel=formalBacktestIdentity(right).label;
    if(leftConditions.missing.length||rightConditions.missing.length){node.innerHTML='<div class="comparison-blocked"><strong>不可直接比较</strong><span>至少一条记录的策略身份不可确认，或缺少区间、随机种子、本金、费用、执行适配器、代码版本、数据快照证据。请选择由当前正式链生成的完整记录。</span></div>';return}
    if(leftConditions.key!==rightConditions.key){var conditionLabels={start_date:'开始日期',end_date:'结束日期',random_seed:'随机种子',initial_capital_cny:'初始资金',cost_scenario_multiplier:'成本情景倍率',top_per_day:'每日候选上限',adapter:'回测适配器',protocol_version:'研究协议',code_commit_sha:'代码版本',data_snapshot_hash:'数据快照'},differences=Object.keys(leftConditions.values).filter(function(key){return String(leftConditions.values[key])!==String(rightConditions.values[key])}).map(function(key){return conditionLabels[key]||key});node.innerHTML='<div class="comparison-blocked"><strong>不可直接比较</strong><span>两次运行的 '+esc(differences.join('、'))+' 不一致。系统不会把不同条件下的结果包装成策略优劣。</span></div>';return}
    var a=formalBacktestMetrics(left),b=formalBacktestMetrics(right);
    function delta(metric,lowerBetter,formatter){var av=a[metric],bv=b[metric];if(av==null||bv==null)return '<span>证据不足</span>';var diff=Number(bv)-Number(av),word=Math.abs(diff)<1e-12?'持平':(lowerBetter?diff<0:diff>0)?'改善':'恶化',kind=word==='改善'?'safe-text':word==='恶化'?'danger-text':'',shown=formatter?formatter(diff):(diff>=0?'+':'')+num(diff,2);return '<span class="'+kind+'">'+word+' '+shown+'</span>'}
    function row(label,key,lowerBetter,format){var av=a[key],bv=b[key],show=function(v){if(v==null)return '—';return format==='pct'?pct(v):format==='money'?money(v):num(v,0)};return '<tr><td>'+esc(label)+'</td><td>'+esc(show(av))+'</td><td>'+esc(show(bv))+'</td><td>'+delta(key,lowerBetter,format==='pct'?function(v){return (v>=0?'+':'')+num(v,2)+'%'}:null)+'</td></tr>'}
    node.innerHTML='<div class="panel-head"><div><p>SAME-CONDITION COMPARISON</p><h3>同条件版本比较</h3></div><span class="badge safe">条件一致</span></div><p class="note">A：'+esc(leftLabel)+' · B：'+esc(rightLabel)+'。改善/恶化按 B 相对 A 计算。</p><div class="table-wrap"><table><thead><tr><th>指标</th><th>A</th><th>B</th><th>B 相对 A</th></tr></thead><tbody>'+row('扣费后收益','netReturn',false,'pct')+row('最大回撤','maxDrawdown',true,'pct')+row('交易样本','sample',false,'count')+row('无法成交','unfilled',true,'count')+row('未平仓 / 未解决退出','unresolved',true,'count')+'</tbody></table></div>';
  }
  function renderFormalBacktestDetail(row){
    var node=el('formalBacktestDetail');if(!node)return;
    row=row||state.activeBacktest||{};if(!row.backtest_uid){node.innerHTML='';return}
    var report=row.result||{},binding=report.strategy_binding||{},identity=formalBacktestIdentity(row),assumptions=report.execution_assumptions||{},promotion=report.promotion_protocol||{},classification=formalRunClassification(row),metrics=formalBacktestMetrics(row),blockers=(promotion.blockers||[]).concat(promotion.reason||[]).filter(Boolean);
    node.innerHTML='<div class="panel-head"><div><p>RUN RESULT</p><h3>运行 '+esc(String(row.backtest_uid).slice(0,12))+'…</h3></div><span class="badge '+classification.kind+'">'+esc(classification.label)+'</span></div><div class="facts">'+[
      fact('精确策略',identity.label),
      fact('配置 / 代码',String(row.config_hash||binding.config_hash||'—').slice(0,16)+' / '+String(row.code_commit_sha||binding.code_commit_sha||'—').slice(0,16)),
      fact('运行区间 / 随机种子',(row.start_date||'—')+' 至 '+(row.end_date||'—')+' / '+(row.random_seed==null?'—':row.random_seed)),
      fact('回测适配器',report.adapter||'—'),
      fact('初始资金',money(assumptions.initial_capital_cny)),
      fact('费用口径',Number(assumptions.cost_scenario_multiplier)===1&&assumptions.fee_profile&&assumptions.fee_profile.usable===true?'已确认费率基准 + 系统 2 倍压力情景':'费率证据缺失，结果阻断'),
      fact('样本 / 未成交 / 未解决',String(metrics.sample==null?'—':metrics.sample)+' / '+String(metrics.unfilled==null?'—':metrics.unfilled)+' / '+String(metrics.unresolved==null?'—':metrics.unresolved)),
      fact('扣费后收益 / 最大回撤',(metrics.netReturn==null?'—':pct(metrics.netReturn))+' / '+(metrics.maxDrawdown==null?'—':pct(metrics.maxDrawdown))),
      fact('最终权益',money(metrics.finalEquity)),
      fact('资格阻断',blockers.join('；')||'无'),
      fact('数据快照',row.data_snapshot_hash||'尚未生成'),
      fact('结果制品',row.result_hash||'尚未生成')
    ].join('')+'</div>';
  }
  function renderFormalPaperEvidence(){
    var ledger=(state.paperLedger||{}).summary,error=state.errors.paperLedger,available=!!ledger&&!error,paperState=available?'模拟账户可读；不可归因到当前策略版本':'模拟账户账本不可用';
    el('researchPaperState').textContent=paperState;el('formalPaperStatus').textContent=paperState;el('formalPaperStatus').className='badge '+(available?'warning':'danger');
    el('formalPaperFacts').innerHTML=!available?fact('读取状态',error?error+'；不能解释为模拟账户表现为零':'模拟账户摘要尚未提供'):[
      fact('统计口径','统一模拟账户混合账本的当前持仓与当日平仓摘要'),
      fact('持仓股票',ledger.position_count==null?'不可用':String(ledger.position_count)+' 只'),
      fact('未平仓批次',ledger.position_lot_count==null?'不可用':String(ledger.position_lot_count)),
      fact('今日已平仓股票',ledger.today_closed_position_count==null?'不可用':String(ledger.today_closed_position_count)+' 只'),
      ledger.total_unrealized_pnl==null?fact('当前浮动盈亏','不可用'):pnlFact('当前浮动盈亏',ledger.total_unrealized_pnl),
      fact('累计成交统计','不可用（接口未提供）'),
      fact('累计已实现盈亏','不可用（接口未提供统一累计统计）'),
      fact('策略证据','暂无当前策略版本的模拟证据；这些汇总不参与策略竞技或前向验证')
    ].join('');
  }
  function renderFormalResearch(){
    if(!el('strategyVersionRows'))return;
    var strategies=state.strategyVersions||[],backtests=((state.researchOperations||{}).backtests)||[],strategyError=state.errors.strategyVersions,historyError=state.errors.backtestHistory,select=el('formalBacktestStrategy'),selectedKey=state.formalStrategyKey||'';
    if(select){
      select.innerHTML='<option value="">选择一个可运行版本</option>'+strategies.map(function(row,index){return {row:row,index:index}}).filter(function(item){return formalBacktestRunnable(item.row)}).map(function(item){var row=item.row,index=item.index,key=String(row.strategy_id||'')+'@@'+String(row.version||'');return '<option value="'+index+'"'+(key===selectedKey?' selected':'')+'>'+esc(formalStrategyLabel(row)+' · '+row.strategy_id+' @ '+row.version)+'</option>'}).join('');
    }
    function missingEvidence(row){var lifecycle=String(row.lifecycle_status||'').toUpperCase(),validation=row.validation||{};if(lifecycle==='RETIRED')return '版本已淘汰，只保留历史';if(lifecycle==='SUSPENDED')return '先处理暂停原因';if(lifecycle==='PAPER_ACTIVE')return '继续积累模拟成交与复盘';if(lifecycle==='PAPER_TRIAL')return validation.paper_gate_passed===true?'等待正式治理决定':'模拟样本或门槛尚未通过';if(lifecycle==='SHADOW')return validation.shadow_gate_passed===true?'等待模拟授权':'影子观察证据尚未通过';if(lifecycle==='OOS_PASSED')return '等待影子观察确认';if(lifecycle==='RESEARCH')return '样本外验收尚未通过';return '尚未形成可用研究证据'}
    function evidence(row){var validation=row.validation||{},parts=[];if(validation.oos_gate_passed===true)parts.push('样本外 PASS');if(validation.shadow_gate_passed===true)parts.push('影子门槛 PASS');if(validation.paper_gate_passed===true)parts.push('模拟门槛 PASS');return parts.join('；')||'尚无通过证据'}
    el('strategyVersionRows').innerHTML=strategyError?empty(6,'策略版本读取失败：'+strategyError):strategies.length?strategies.map(function(row,index){var supported=formalAdapterSupported(row),runnable=formalBacktestRunnable(row),adapterText=!supported?(row.backtest_adapter_reason||'没有可复算回测适配器'):row.formal_fee_coverage_usable!==true?'已确认 ETF 费率覆盖不可用':!row.latest_validated_session?'登记 ETF 数据尚未完整覆盖':row.formal_data_contract_status==='BLOCKED'?'依赖窗口数据不完整：'+String(row.formal_data_contract_reason||'缺少可核验证据'):!row.latest_runnable_session?'数据与费率没有共同可运行交易日':(row.backtest_adapter_reason||'可运行正式回测');return '<tr><td>'+esc(formalStrategyLabel(row))+'<small class="cell-sub">'+esc(row.strategy_id||'—')+'</small></td><td><code>'+esc(row.version||'—')+'</code><small class="cell-sub">配置 '+esc(String(row.config_hash||'—').slice(0,12))+' · 代码 '+esc(String(row.code_commit_sha||'—').slice(0,12))+'</small></td><td>'+esc(formalLifecycle(row.lifecycle_status))+'<small class="cell-sub '+(runnable?'safe-text':'danger-text')+'">'+esc(adapterText)+'</small></td><td>'+esc(evidence(row))+'</td><td class="reason">'+esc(runnable?missingEvidence(row):adapterText)+'</td><td><button type="button" class="evidence-button" data-formal-strategy-index="'+index+'"'+(runnable?'':' disabled title="正式回测所需数据或费率覆盖不可用"')+'>回测此版本</button> <button type="button" class="evidence-button" data-open-strategy-pool="'+esc(row.strategy_id||'')+'" title="打开当前统一策略票池；只有存在同名策略时才自动筛选">看当前票池</button></td></tr>'}).join(''):empty(6,'未运行：系统尚未登记策略版本');
    var completed=backtests.filter(function(x){return String(x.status).toUpperCase()==='COMPLETED'}).length,failed=backtests.filter(function(x){return String(x.status).toUpperCase()==='FAILED'}).length,running=backtests.length-completed-failed;
    el('formalBacktestSummary').textContent=historyError?'回测记录不可用':backtests.length?backtests.length+' 次独立记录 · '+running+' 运行中 · '+failed+' 失败':'未运行';
    el('formalBacktestSummary').className='badge '+(historyError?'danger':running?'warning':'');
    el('researchBacktestState').textContent=historyError?'不可用':backtests.length?completed+' 已完成 / '+running+' 运行中 / '+failed+' 失败':'未运行';
    var v=state.validation||{},validationError=state.errors.validation,oosState=validationError?'不可用':v.result_status==='PASS'?'已通过':v.result_status?v.result_status:'证据不足或未运行';el('researchOosState').textContent=oosState;
    renderFormalPaperEvidence();
    var availableIds=backtests.filter(function(row){var classification=formalRunClassification(row);return formalBacktestIdentity(row).verified&&(classification.code==='PASS'||classification.code==='INSUFFICIENT'||classification.code==='NOT_QUALIFIED')}).map(function(x){return String(x.backtest_uid||'')});state.compareBacktests=(state.compareBacktests||[]).filter(function(uid){return availableIds.indexOf(uid)>=0});
    el('formalBacktestRows').innerHTML=historyError?empty(8,'回测历史读取失败：'+historyError):backtests.length?backtests.map(function(row){var identity=formalBacktestIdentity(row),classification=formalRunClassification(row),uid=String(row.backtest_uid||''),canCompare=identity.verified&&(classification.code==='PASS'||classification.code==='INSUFFICIENT'||classification.code==='NOT_QUALIFIED'),checked=state.compareBacktests.indexOf(uid)>=0?' checked':'';return '<tr><td><input type="checkbox" data-compare-backtest="'+esc(uid)+'" aria-label="选择运行 '+esc(uid)+' 进行比较"'+checked+(canCompare?'':' disabled')+'></td><td><code>'+esc(String(row.backtest_uid||'—').slice(0,12))+'</code><small class="cell-sub">种子 '+esc(row.random_seed==null?'—':row.random_seed)+'</small></td><td>'+esc(identity.label)+'</td><td>'+esc((row.start_date||'—')+' 至 '+(row.end_date||'—'))+'</td><td><span class="badge '+classification.kind+'">'+esc(classification.code==='FAILED'?'运行失败':String(row.status||'—'))+'</span>'+(row.error_message?'<small class="cell-sub danger-text">'+esc(row.error_message)+'</small>':'')+'</td><td>'+esc(classification.label)+'</td><td>'+esc(displayDateTime(row.finished_at||row.started_at))+'</td><td><button type="button" class="evidence-button" data-backtest-uid="'+esc(uid)+'">查看证据</button></td></tr>'}).join(''):empty(8,'未运行：选择一个精确版本后发起研究回测');
    el('formalBacktestCompareCount').textContent=state.compareBacktests.length?('已选择 '+state.compareBacktests.length+' / 2 条记录'):'选择两条已完成记录进行比较';el('formalBacktestCompare').disabled=state.compareBacktests.length!==2;
    var message=el('formalBacktestMessage');message.hidden=!state.formalBacktestMessage;message.textContent=state.formalBacktestMessage||'';
    renderFormalBacktestBinding();renderFormalBacktestComparison();renderFormalBacktestDetail();
  }
  function selectedFormalStrategy(){var select=el('formalBacktestStrategy'),index=select&&select.value!==''?Number(select.value):-1;return index>=0?(state.strategyVersions||[])[index]||null:null}
  function renderFormalBacktestBinding(){var row=selectedFormalStrategy(),node=el('formalBacktestBinding');if(!node)return;if(!row){node.textContent='选择策略后显示精确版本、配置哈希和代码版本。';return}state.formalStrategyKey=String(row.strategy_id||'')+'@@'+String(row.version||'');node.textContent='本次绑定：'+formalStrategyLabel(row)+' · '+row.strategy_id+' @ '+row.version+' · 配置 '+String(row.config_hash||'—').slice(0,16)+' · 代码 '+String(row.code_commit_sha||'—').slice(0,16)+' · 完整数据截止 '+String(row.latest_validated_session||'不可用')+' · 可运行截止 '+String(row.latest_runnable_session||'不可用')+' · 费率覆盖 '+formalFeeCoverageLabel(row)+' · 运行时须绑定已确认费率；系统另做 2 倍压力情景 · '+(row.backtest_adapter_reason||'回测适配器状态未知')}
  ...
  window.addEventListener('message',function(event){
    ...
    var previousView=state.activeView,incomingView=String(event.data.view||'overview'),incomingDate=String(event.data.requested_date||''),incomingMode=normalizedRequestedDateMode(event.data.date_mode),dateChanged=!!incomingDate&&incomingDate!==state.requestedDate,modeChanged=incomingMode!==state.requestedDateMode,viewChanged=incomingView!==previousView;
    state.requestedDate=incomingDate||state.requestedDate;state.requestedDateMode=incomingMode;applyFilters(event.data.filters||{});activateView(incomingView);
    if(dateChanged||modeChanged||viewChanged)load();else{if(incomingView==='hypotheses')renderHypotheses();if(incomingView==='candidates')renderCandidates();notifyParentResize()}
  });
  if(window.ResizeObserver)new ResizeObserver(notifyParentResize).observe(document.documentElement);
  var candidateTimer=null;
  function reloadCandidates(){state.candidatePage=1;state.researchCandidatePage=1;state.researchStockPool={};state.researchStockPoolRequestDate='';var day=el('candidateDate').value;if(day){state.requestedDate=day;state.requestedDateMode='EXPLICIT_HISTORICAL'}syncFilters();load()}
  function refreshCandidatePools(){
    if(document.hidden||state.activeView!=='candidates'||state.loading||state.candidateRefreshPending)return;
    var formalTarget=String(state.requestedDate||el('candidateDate').value||(state.stockPool||{}).requested_trade_date||'').slice(0,10);if(!/^\d{4}-\d{2}-\d{2}$/.test(formalTarget))return;
    var refreshSeq=++state.candidateRefreshSeq,loadSeq=state.loadSeq,dateMode=state.requestedDateMode;state.candidateRefreshPending=true;
    Promise.all([
      requestResult('candidateDailyResult',api3('/daily-result?trade_date='+encodeURIComponent(formalTarget),60000),{}),
      requestResult('candidateDailyDelivery',fetchJson('/api/scheduler/daily-delivery?trade_date='+encodeURIComponent(formalTarget)),{})
    ]).then(function(results){
      if(refreshSeq!==state.candidateRefreshSeq||loadSeq!==state.loadSeq||dateMode!==state.requestedDateMode||formalTarget!==String(state.requestedDate||el('candidateDate').value||'').slice(0,10))return null;
      var dailyResult=results[0],deliveryResult=results[1];
      if(dailyResult.ok){var daily=dailyResult.value||{},dailyDate=String(((daily.context||{}).requested_date)||((daily.context||{}).requested_trade_date)||((daily.stock_pool||{}).requested_trade_date)||((daily.stock_pool||{}).trade_date)||'').slice(0,10);if(daily.context&&daily.overview&&daily.stock_pool&&(!dailyDate||dailyDate===formalTarget)){state.dailyResult=daily;state.context=daily.context;state.overview=daily.overview;state.stockPool=daily.stock_pool;delete state.errors.candidateDailyRefresh}else state.errors.candidateDailyRefresh='每日原子结果与当前请求日不一致'}else state.errors.candidateDailyRefresh=dailyResult.error;
      if(deliveryResult.ok){state.dailyDelivery=deliveryResult.value||{};delete state.errors.candidateDailyDelivery}else state.errors.candidateDailyDelivery=deliveryResult.error;
      var researchTarget=researchPoolRequestDate(dateMode,formalTarget,state.dailyResult,state.marketClock);if(!/^\d{4}-\d{2}-\d{2}$/.test(researchTarget))return null;
      if(researchTarget!==state.researchStockPoolRequestDate)state.researchStockPool={};state.researchStockPoolRequestDate=researchTarget;
      return requestResult('candidateResearchPool',api3('/research/stock-pool?trade_date='+encodeURIComponent(researchTarget),60000),{}).then(function(researchResult){return {researchResult:researchResult,researchTarget:researchTarget}})
    }).then(function(result){
      if(!result||refreshSeq!==state.candidateRefreshSeq||loadSeq!==state.loadSeq||dateMode!==state.requestedDateMode||formalTarget!==String(state.requestedDate||el('candidateDate').value||'').slice(0,10)||result.researchTarget!==state.researchStockPoolRequestDate)return;
      var researchResult=result.researchResult,researchTarget=result.researchTarget;if(researchResult.ok&&String((researchResult.value||{}).trade_date||'').slice(0,10)===researchTarget){state.researchStockPool=researchResult.value||{};delete state.errors.researchStockPool}else{state.researchStockPool={};state.errors.researchStockPool=researchResult.ok?'研究观察池与当前请求日不一致':researchResult.error}
      el('updatedAt').textContent='票池自动检查于 '+new Date().toLocaleTimeString('zh-CN',{hour12:false});renderAll();activateView('candidates');notifyParentResize();
    }).finally(function(){if(refreshSeq===state.candidateRefreshSeq)state.candidateRefreshPending=false})
  }
  ...
  function applyFormalStrategyDefaults(){
    var row=selectedFormalStrategy();renderFormalBacktestBinding();if(!row)return;
    var isEtf=String(row.instrument_scope||'').toUpperCase()==='EXCHANGE_TRADED_FUND';
    var adapterStart=String(row.backtest_adapter_minimum_start_date||''),feeStart=String(row.earliest_fee_covered_start||''),minimumStart=adapterStart>feeStart?adapterStart:feeStart,startInput=el('formalBacktestStart');startInput.min=minimumStart;startInput.value=minimumStart;
    var maximumEnd=String(row.latest_runnable_session||''),endInput=el('formalBacktestEnd');endInput.max=maximumEnd;endInput.value=maximumEnd;
    el('formalBacktestCapital').value=isEtf?'200000':'1000000';
  }
  function fetchFormalBacktest(backtestUid){
    if(!backtestUid)return;
    var requestedUid=String(backtestUid),requestSeq=(Number(state.formalBacktestRequestSeq)||0)+1;
    state.formalBacktestRequestSeq=requestSeq;state.requestedBacktestUid=requestedUid;state.activeBacktest={};
    function isCurrentRequest(){return state.formalBacktestRequestSeq===requestSeq&&state.requestedBacktestUid===requestedUid}
    state.formalBacktestMessage='正在读取该次回测证据…';renderFormalResearch();
    return api2('/research/backtests/'+encodeURIComponent(requestedUid)).then(function(payload){
      if(!isCurrentRequest())return;
      var result=unwrap(payload)||{};
      if(String(result.backtest_uid||'')!==requestedUid)throw new Error('回测证据运行标识与请求不一致');
      state.activeBacktest=result;state.formalBacktestMessage='已读取独立运行 '+requestedUid.slice(0,12)+'… 的结果制品。';renderFormalResearch();notifyParentResize();
    }).catch(function(err){if(!isCurrentRequest())return;state.formalBacktestMessage='结果读取失败：'+errorText(err);renderFormalResearch()})
  }
  function pollFormalBacktestJob(jobId,remaining,delayMs){
    setTimeout(function(){
      api2('/jobs/'+encodeURIComponent(jobId)).then(function(payload){
        var job=unwrap(payload)||{},jobState=String(job.status||'UNKNOWN').toUpperCase(),pending=/^(PENDING|RUNNING|QUEUED|CREATED)$/.test(jobState),terminal=/^(COMPLETED|FAILED|CANCELLED|CANCELED)$/.test(jobState);
        if(pending&&remaining>0){state.formalBacktestMessage='正式回测 '+(jobState==='RUNNING'?'运行中':'排队中')+' · job '+jobId.slice(0,12)+'…';renderFormalResearch();pollFormalBacktestJob(jobId,remaining-1,delayMs);return}
        if(pending||!terminal){state.activeBacktestJobId=jobId;el('formalBacktestRun').disabled=true;state.formalBacktestMessage=(pending?'前台快速轮询已超时，任务仍在后台运行':'任务状态暂时无法确认')+' · job '+jobId+'；按钮保持禁用，已切换低频追踪。';renderFormalResearch();pollFormalBacktestJob(jobId,30,10000);return}
        state.activeBacktestJobId='';el('formalBacktestRun').disabled=false;
        if(jobState==='COMPLETED'&&job.result_ref){state.formalBacktestMessage='正式回测已完成，正在读取独立结果…';loadFormalResearch(state.loadSeq);fetchFormalBacktest(String(job.result_ref));return}
        state.formalBacktestMessage=jobState==='FAILED'?'运行失败：'+(job.error_message||job.error_code||'未返回失败原因'):'任务状态 '+jobState+'；请稍后刷新历史记录';renderFormalResearch();
      }).catch(function(err){state.activeBacktestJobId=jobId;el('formalBacktestRun').disabled=true;state.formalBacktestMessage='任务状态读取失败：'+errorText(err)+' · job '+jobId+'；按钮保持禁用并继续低频追踪。';renderFormalResearch();pollFormalBacktestJob(jobId,30,10000)})
    },delayMs||2000)
  }
  function runFormalBacktest(event){
    event.preventDefault();var form=el('formalBacktestForm'),strategyRow=selectedFormalStrategy(),button=el('formalBacktestRun');if(state.activeBacktestJobId){button.disabled=true;state.formalBacktestMessage='已有正式回测任务正在运行或状态待确认 · job '+state.activeBacktestJobId+'；请勿重复提交。';renderFormalResearch();return}if(!form.reportValidity()||!strategyRow)return;if(!formalBacktestRunnable(strategyRow)){state.formalBacktestMessage='该版本的登记 ETF 完整数据或已确认费率覆盖不可用，不能提交正式回测。';renderFormalResearch();return}
    var request={strategy_id:String(strategyRow.strategy_id||''),strategy_version:String(strategyRow.version||''),start_date:el('formalBacktestStart').value,end_date:el('formalBacktestEnd').value,initial_capital:Number(el('formalBacktestCapital').value),top_per_day:Number(el('formalBacktestTop').value),random_seed:Number(el('formalBacktestSeed').value)};
    if(request.end_date<request.start_date){state.formalBacktestMessage='结束日期不能早于开始日期。';renderFormalResearch();return}
    button.disabled=true;state.formalBacktestMessage='正在创建独立研究回测；不会覆盖旧记录…';renderFormalResearch();
    postJsonBody('/api/v2/research/backtests',request).then(function(payload){var job=unwrap(payload)||{},jobId=String(job.job_id||'');if(!jobId)throw new Error('服务端未返回 job_id');state.activeBacktestJobId=jobId;state.formalBacktestMessage='已提交独立运行 '+String(job.run_request_uid||'').slice(0,12)+'… · job '+jobId.slice(0,12)+'…';renderFormalResearch();pollFormalBacktestJob(jobId,90,2000)}).catch(function(err){button.disabled=false;state.formalBacktestMessage='创建失败：'+errorText(err);renderFormalResearch()})
  }
  ['strategyFilter','statusFilter'].forEach(function(id){el(id).addEventListener('change',function(){state.candidatePage=1;state.researchCandidatePage=1;syncFilters();renderCandidates();notifyParentResize()})});
  el('candidateDate').addEventListener('change',reloadCandidates);
  el('candidateSearch').addEventListener('input',function(){clearTimeout(candidateTimer);candidateTimer=setTimeout(function(){state.candidatePage=1;state.researchCandidatePage=1;syncFilters();renderCandidates();notifyParentResize()},250)});
  ...
  el('candidateReconstructionPrev').addEventListener('click',function(){if(state.researchCandidatePage>1){state.researchCandidatePage-=1;renderCandidates();notifyParentResize()}});
  el('candidateReconstructionNext').addEventListener('click',function(){state.researchCandidatePage+=1;renderCandidates();notifyParentResize()});
  ...
  el('formalBacktestStrategy').addEventListener('change',applyFormalStrategyDefaults);
  el('formalBacktestForm').addEventListener('submit',runFormalBacktest);
  el('strategyVersionRows').addEventListener('click',function(event){var runButton=event.target.closest('[data-formal-strategy-index]'),poolButton=event.target.closest('[data-open-strategy-pool]');if(runButton){el('formalBacktestStrategy').value=runButton.dataset.formalStrategyIndex;applyFormalStrategyDefaults();el('formalBacktestForm').scrollIntoView({behavior:'smooth',block:'center'});return}if(poolButton){activateView('candidates',{userInitiated:true});var option=Array.prototype.find.call(el('strategyFilter').options,function(x){return x.value===poolButton.dataset.openStrategyPool});el('strategyFilter').value=option?option.value:'';syncFilters();load()}});
  el('formalBacktestRows').addEventListener('click',function(event){var button=event.target.closest('[data-backtest-uid]');if(button)fetchFormalBacktest(button.dataset.backtestUid)});
  el('formalBacktestRows').addEventListener('change',function(event){var checkbox=event.target.closest('[data-compare-backtest]');if(!checkbox)return;var uid=String(checkbox.dataset.compareBacktest||''),selected=state.compareBacktests||[];if(checkbox.checked){if(selected.length>=2&&selected.indexOf(uid)<0){checkbox.checked=false;state.formalBacktestMessage='最多选择两条回测记录进行比较。';renderFormalResearch();return}if(selected.indexOf(uid)<0)selected=selected.concat(uid)}else selected=selected.filter(function(value){return value!==uid});state.compareBacktests=selected;renderFormalResearch();notifyParentResize()});
  el('formalBacktestCompare').addEventListener('click',function(){renderFormalBacktestComparison();var node=el('formalBacktestComparison');if(node&&node.innerHTML)node.scrollIntoView({behavior:'smooth',block:'nearest'});notifyParentResize()});
  el('runDaily').addEventListener('click',function(){runDailyAction(this)});
  el('refresh').addEventListener('click',load);document.addEventListener('visibilitychange',function(){if(!document.hidden)refreshCandidatePools()});setInterval(refreshWatchlistStrategy,60000);setInterval(refreshCandidatePools,30000);var formalStartDate=new Date();formalStartDate.setFullYear(formalStartDate.getFullYear()-1);el('formalBacktestStart').value=formalStartDate.getFullYear()+'-'+String(formalStartDate.getMonth()+1).padStart(2,'0')+'-'+String(formalStartDate.getDate()).padStart(2,'0');state.requestedDate=requestedFrameDate()||state.requestedDate;state.requestedDateMode=requestedFrameDateMode();activateView(requestedView());load();
})();
