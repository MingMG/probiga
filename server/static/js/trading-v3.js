(function(){
  ...
  function recommendationRange(row,lowKey,highKey){row=row||{};var low=firstNumber([row[lowKey]]),high=firstNumber([row[highKey]]);return low==null&&high==null?null:{low:low,high:high}}
  ...
  function holdingStrategyPayload(promise){return promise.then(function(v){if(String(v.status||'ok').toLowerCase()==='error')throw new Error(v.error||'持仓策略不可用');return {rows:Array.isArray(v.data)?v.data:[],summary:v.summary||{},trade_date:v.trade_date,historical_read_only:v.historical_read_only,knowledge_cutoff:v.knowledge_cutoff,execution_authority:v.execution_authority,status:v.status}})}
  function recommendationPayload(promise){return promise.then(function(v){if(v.error)throw new Error(v.error);return {items:Array.isArray(v.data)?v.data:[],date:v.date,total:v.total,error:''}})}
  ...
  function load(){
    ...
    if(view==='candidates'){add('stockPool',api3(withDate('/stock-pool')),{});add('recommendations',recommendationPayload(fetchJson('/api/hot-data/recommended-stocks'+(state.requestedDate?'?'+dateParam():''))),{})}
  ...
  function renderWatchlistStrategy(){
    ...
    function render(statusId,summaryId,rowsId){
      ...
      summaryNode.innerHTML=error?fact('读取错误',error+'；不能把策略缺失解释为继续持有'):[fact('自选持仓',String(summary.holding_count||0)+' 只'),fact('立即卖出',String(summary.sell_count||0)+' 只'),fact('分批减仓',String(summary.reduce_count||0)+' 只'),fact('继续持有',String(summary.hold_count||0)+' 只'),fact('证据不完整',String(summary.wait_data_count||0)+' 只'),fact('策略权限','ADVISORY_ONLY · 不自动下单')].join('');
      rowsNode.innerHTML=error?empty(8,'持仓策略不可用：'+error):rows.length?rows.map(function(x){var sell=x.sell_plan||{},emergency=x.emergency_exit||{},actionClass=x.exit_intent==='SELL'?'danger-text':x.exit_intent==='REDUCE'||x.exit_intent==='WAIT_DATA'?'warning-text':'safe-text',costLatest=money(x.cost_price)+' / '+money(x.latest_price)+(x.pnl_pct==null?'':' · '+(Number(x.pnl_pct)>0?'+':'')+pct(x.pnl_pct)),buy=(x.buy_plan||{}),cutoff=displayDateTime(x.knowledge_cutoff||x.evaluated_at);return '<tr data-intent="'+esc(x.exit_intent||'WAIT_DATA')+'"><td>'+security(x.stock_code,x.short_name)+'<small class="holding-meta">'+esc((x.position_date||'日期未知')+' · '+num(x.shares,0)+' 股')+'</small></td><td class="'+pnlClass(x.pnl)+'">'+esc(costLatest)+'</td><td><strong class="'+actionClass+'">'+esc(x.action||'—')+'</strong><small class="holding-reason">'+esc(x.reason||'—')+'</small></td><td>'+esc(priceRange(buy.range))+'<small class="holding-plan-label">'+esc(buy.label||'—')+'</small></td><td>'+esc(priceRange(sell.range))+'<small class="holding-plan-label">'+esc(sell.label||'—')+'</small></td><td class="'+(emergency.direct?'danger-text':'')+'">'+esc(emergency.price==null?'—':money(emergency.price))+'<small class="holding-plan-label">'+esc(emergency.label||'—')+'</small></td><td>'+esc(x.next_session_plan||'—')+'</td><td>'+esc(cutoff)+'<small class="holding-plan-label">'+esc(payload.historical_read_only?'历史只读复盘':'每 60 秒低频更新')+'</small></td></tr>'}).join(''):empty(8,'自选股里尚未登记“成本价 + 持仓股数”；登记后会从买入日起持续给出操作建议');
    }
  ...
  function renderCandidates(){
    ...
    function plans(x){var rec=x.recommendation||{},buy=recommendationRange(rec,'entry_price_low','entry_price_high'),sell={low:firstNumber([rec.take_profit_1,rec.trend_reduce_price]),high:firstNumber([rec.take_profit_2,rec.resistance_price,rec.take_profit_1])},stop=firstNumber([rec.stop_loss_price,rec.trend_stop_price]),signal=String(rec.signal_status||rec.recommend_status||'').toUpperCase(),emergency=signal==='SELL_ALERT'?'SELL_ALERT：直接退出':stop==null?'暂无可信退出线':'跌破 '+money(stop)+' 直接退出';return {buy:priceRange(buy),sell:priceRange(sell),emergency:emergency,signal:signal}}
    function card(x){var calibrated=hasCalibratedExpectation(x),reasons=(x.reasons||[]).join('；')||status(x.forecast_status),fullTheme=themeText(x),plan=plans(x);return '<article class="candidate-card"><div class="candidate-card-head"><span class="candidate-rank">#'+esc(num(x.rank_no,0))+'</span>'+security(x.stock_code,x.short_name)+'<span class="badge '+(x.target?'safe':x.rejection?'danger':'warning')+'">'+esc(status(x.forecast_status))+'</span></div><div class="candidate-card-route"><strong>'+esc(strategyText(x))+'</strong><span title="'+esc(fullTheme)+'">'+esc(themeSummary(x))+'</span></div><div class="candidate-card-plans"><div><span>买入范围</span><strong>'+esc(plan.buy)+'</strong></div><div><span>卖出范围</span><strong>'+esc(plan.sell)+'</strong></div><div><span>突发退出</span><strong class="'+(plan.signal==='SELL_ALERT'?'danger-text':'')+'">'+esc(plan.emergency)+'</strong></div></div><div class="candidate-card-metrics"><div><span>综合分</span><strong>'+num(x.raw_score,3)+'</strong></div><div><span>研究净期望</span><strong class="'+(calibrated&&Number(x.expected_return_net_pct)>0?'safe-text':calibrated?'':'warning-text')+'">'+(calibrated?pct(x.expected_return_net_pct):'未校准')+'</strong></div><div><span>权限</span><strong class="warning-text">RESEARCH_ONLY</strong></div><div><span>批次</span><strong>'+esc(String((state.stockPool||{}).run_uid||'—').slice(0,10))+'</strong></div></div><p class="candidate-card-reason"><b>判断依据：</b>'+esc(reasons)+'</p></article>'}
    ...
    el('candidateRows').innerHTML=pageRows.length?pageRows.map(function(x){var calibrated=hasCalibratedExpectation(x),plan=plans(x);return '<tr><td>'+num(x.rank_no,0)+'</td><td>'+security(x.stock_code,x.short_name)+'</td><td>'+esc(strategyText(x))+'</td><td title="'+esc(themeText(x))+'">'+esc(themeSummary(x))+'</td><td>'+esc(plan.buy)+'</td><td>'+esc(plan.sell)+'</td><td class="'+(plan.signal==='SELL_ALERT'?'danger-text':'')+'">'+esc(plan.emergency)+'</td><td>'+num(x.raw_score,3)+'</td><td class="'+(calibrated&&Number(x.expected_return_net_pct)>0?'safe-text':calibrated?'':'warning-text')+'">'+(calibrated?pct(x.expected_return_net_pct):'未校准')+'</td><td>'+(calibrated?pct(Number(x.probability_positive)*100,1):'未校准')+'</td><td>—</td><td>—</td><td>—</td><td>'+esc(status(x.forecast_status))+'</td><td class="reason">'+esc((x.reasons||[]).join('；'))+' · RESEARCH_ONLY</td></tr>'}).join(''):decisionEmptyFor('stockPool',15,'已验证批次在该筛选条件下没有记录');
  ...
  function reloadCandidates(){state.candidatePage=1;var day=el('candidateDate').value,query=day?'?trade_date='+encodeURIComponent(day):'';syncFilters();Promise.all([requestResult('stockPool',api3('/stock-pool'+query),{}),requestResult('recommendations',recommendationPayload(fetchJson('/api/hot-data/recommended-stocks'+query)),{})]).then(function(results){results.forEach(function(result){if(result.ok){delete state.errors[result.key];state[result.key]=result.value}else{state.errors[result.key]=result.error;state[result.key]=result.fallback}});renderCandidates();renderTruthContext();notifyParentResize()})}
