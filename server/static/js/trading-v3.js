(function(){
  ...
  function renderCandidates(){
    ...
    var pool=state.stockPool||{},clock=state.marketClock||{},latestFormalDate=String(clock.recommendation_trade_date||clock.latest_data_date||'').slice(0,10);if(clock.latest_data_date&&latestFormalDate>String(clock.latest_data_date).slice(0,10))latestFormalDate=String(clock.latest_data_date).slice(0,10);var historicalFallback=pool.is_historical_fallback===true,governanceDeferred=pool.governance_deferred===true||pool.activation_enabled===false,formalTruth=stockPoolFormalTruth(pool,state.requestedDate||pool.requested_trade_date,latestFormalDate),formalCurrent=formalTruth.ready===true,researchOnlyDisplay=!formalCurrent;
