(function(){
  'use strict';
  var state={dailyResult:{},dailyDelivery:{},context:{},marketClock:{},readiness:{},overview:{},stockPool:{},auctionGate:{},watchlistStrategy:{},forecasts:[],oversold:[],targets:[],validation:null,recall:null,learning:{},account:{},paperLedger:{},positions:[],orders:[],runs:[],intraday:{},hypotheses:[],hypothesisDetail:null,tasks:[],dataEvidence:{},lineage:{},researchGovernance:{},batchDiff:{},decisionIntelligence:{},horizonValidation:{},counterfactualResearch:{},shadowPreview:{},strategyVersions:[],researchOperations:{},activeBacktest:{},compareBacktests:[],formalBacktestMessage:'',researchErrors:{},candidatePage:1,actionMessage:'',errors:{},staleKeys:{},loadedKeys:{},pendingKeys:{},requestedDate:'',activeView:'overview',filters:{},activeJobId:'',activeBacktestJobId:'',loadSeq:0,loading:true,truthLoading:true};
  ...
  function renderCandidates(){
    ...
    el('candidateSummary').textContent=(formalCurrent?(deliveryTruth.status==='DEGRADED'?'DEGRADED · 核心票池已验证 · ':pool.is_as_of_fallback===true?'PASS · 最新成功批次 · ':'PASS · 当前正式票池 · '):governanceDeferred?'治理延期 · 只读研究 · ':historicalFallback?'历史只读 · ':researchDefaultFallback?'研究观察 · ':'')+'请求日 '+requestedDate+' · 决策日 '+decisionDate+' · 数据日 '+dataDate+' · 执行日 '+(poolExecutionDate||'—')+' · 当前显示 '+rows.length+' 条'+(formalCurrent?'':' · 阻断 '+formalTruth.reasonCode);el('candidatePageStatus').textContent='第 '+state.candidatePage+' / '+pageCount+' 页';el('candidatePrev').disabled=state.candidatePage<=1;el('candidateNext').disabled=state.candidatePage>=pageCount;el('candidatePager').hidden=rows.length<=pageSize;
  }
  ...
  el('refresh').addEventListener('click',load);setInterval(refreshWatchlistStrategy,60000);var formalStartDate=new Date();formalStartDate.setFullYear(formalStartDate.getFullYear()-1);el('formalBacktestStart').value=formalStartDate.getFullYear()+'-'+String(formalStartDate.getMonth()+1).padStart(2,'0')+'-'+String(formalStartDate.getDate()).padStart(2,'0');state.requestedDate=requestedFrameDate()||state.requestedDate;activateView(requestedView());load();
})();
