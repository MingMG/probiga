(function(){
  ...
  var actionabilityNames={BUY_ZONE:'允许买入区间',PAPER_ONLY:'仅模拟研究',WAIT_TRIGGER:'等待触发',REJECTED:'组合已拒绝',RESEARCH_ONLY:'研究记录'};
  ...
  function holdingStrategyPayload(promise){return promise.then(function(v){if(String(v.status||'ok').toLowerCase()==='error')throw new Error(v.error||'持仓策略不可用');return {rows:Array.isArray(v.data)?v.data:[],summary:v.summary||{},trade_date:v.trade_date,historical_read_only:v.historical_read_only,knowledge_cutoff:v.knowledge_cutoff,execution_authority:v.execution_authority,decision_run_uid:v.decision_run_uid,decision_at:v.decision_at,decision_data_date:v.decision_data_date,decision_session_date:v.decision_session_date,status:v.status}})}
  ...
  function renderWatchlistStrategy(){
    ...
    function render(statusId,summaryId,rowsId){
      ...
      summaryNode.innerHTML=error?fact('读取错误',error+'；不能把策略缺失解释为继续持有'):[fact('自选持仓',String(summary.holding_count||0)+' 只'),fact('立即卖出',String(summary.sell_count||0)+' 只'),fact('分批减仓',String(summary.reduce_count||0)+' 只'),fact('继续持有',String(summary.hold_count||0)+' 只'),fact('证据不完整',String(summary.wait_data_count||0)+' 只'),fact('统一批次',payload.decision_run_uid?String(payload.decision_run_uid).slice(0,10):'未绑定'),fact('决策日 / 数据日',String(payload.decision_session_date||'—').slice(0,10)+' / '+String(payload.decision_data_date||'—').slice(0,10)),fact('策略权限','ADVISORY_ONLY · 不自动下单')].join('');
  ...
  function renderCandidates(){
    ...
    if(!el('candidateDate').value){var defaultDate=(state.stockPool||{}).decision_session_date||(state.context||{}).decision_session_date||(state.stockPool||{}).trade_date||'';if(defaultDate)el('candidateDate').value=String(defaultDate).slice(0,10)}
    var sf=el('strategyFilter').value,st=el('statusFilter').value,q=el('candidateSearch').value.trim().toLowerCase();
    var source=((state.stockPool||{}).items||[]).map(function(item){var rejection=item.rejection||{},actionability=String(item.actionability||((item.action_plan||{}).actionability)||'RESEARCH_ONLY');return Object.assign({},item,{short_name:item.stock_name,strategy_key:(item.strategy_keys||[])[0]||'',forecast_status:actionability,reasons:[rejection.reason||rejection.reason_code].concat(item.reasons||[]).filter(Boolean)})}).filter(function(item){return item.actionability!=='RESEARCH_ONLY'});
    var rows=source.filter(function(x){return(!sf||(x.strategy_keys||[]).indexOf(sf)>=0)&&(st?x.actionability===st:x.actionability!=='REJECTED')&&(!q||String(x.stock_code).toLowerCase().indexOf(q)>=0||String(x.short_name).toLowerCase().indexOf(q)>=0||themeText(x).toLowerCase().indexOf(q)>=0)});
    ...
    el('candidateRows').innerHTML=pageRows.length?pageRows.map(function(x){var calibrated=hasCalibratedExpectation(x),plan=plans(x);return '<tr><td>'+num(x.rank_no,0)+'</td><td>'+security(x.stock_code,x.short_name)+'</td><td>'+esc(strategyText(x))+'</td><td title="'+esc(themeText(x))+'">'+esc(themeSummary(x))+'</td><td>'+esc(plan.buy)+'</td><td>'+esc(plan.sell)+'</td><td>'+esc(plan.emergency)+'</td><td>'+num(x.raw_score,3)+'</td><td class="'+(calibrated&&Number(x.expected_return_net_pct)>0?'safe-text':calibrated?'':'warning-text')+'">'+(calibrated?pct(x.expected_return_net_pct):'未校准')+'</td><td>'+(calibrated?pct(Number(x.probability_positive)*100,1):'未校准')+'</td><td>'+esc(calibrated?ratio(x.profit_factor):'未校准')+'</td><td>'+esc(calibrated?ratio(x.payoff_ratio):'未校准')+'</td><td>'+esc(calibrated?num(x.sample_count,0):'0')+'</td><td>'+esc(actionabilityNames[plan.actionability]||plan.actionability)+'</td><td class="reason">'+esc((x.reasons||[]).join('；'))+' · V3_IMMUTABLE_RUN</td></tr>'}).join(''):decisionEmptyFor('stockPool',15,'同批次没有可操作候选；条件未满足和数据不足记录已移到研究审计，不冒充策略池');
    var pool=state.stockPool||{},decisionDate=String(pool.decision_session_date||'—').slice(0,10),dataDate=String(pool.trade_date||'—').slice(0,10);el('candidateSummary').textContent='决策日 '+decisionDate+' · 数据日 '+dataDate+' · 可见 '+rows.length+' 条 · 当前 '+(rows.length?start+1:0)+'–'+Math.min(start+pageSize,rows.length);el('candidatePageStatus').textContent='第 '+state.candidatePage+' / '+pageCount+' 页';el('candidatePrev').disabled=state.candidatePage<=1;el('candidateNext').disabled=state.candidatePage>=pageCount;el('candidatePager').hidden=rows.length<=pageSize;
  }
  ...
  function reloadCandidates(){state.candidatePage=1;var day=el('candidateDate').value,query=day?'?trade_date='+encodeURIComponent(day):'';syncFilters();Promise.all([requestResult('stockPool',api3('/stock-pool'+query),{})]).then(function(results){results.forEach(function(result){if(result.ok){delete state.errors[result.key];state[result.key]=result.value}else{state.errors[result.key]=result.error;state[result.key]=result.fallback}});renderCandidates();renderTruthContext();notifyParentResize()})}
