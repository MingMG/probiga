(function(){
  'use strict';
  var state={context:{},readiness:{},overview:{},stockPool:{},forecasts:[],oversold:[],targets:[],validation:null,recall:null,learning:{},account:{},paperLedger:{},positions:[],orders:[],runs:[],intraday:{},hypotheses:[],hypothesisDetail:null,tasks:[],dataEvidence:{},lineage:{},researchGovernance:{},batchDiff:{},decisionIntelligence:{},horizonValidation:{},counterfactualResearch:{},shadowPreview:{},researchErrors:{},candidatePage:1,actionMessage:'',errors:{},staleKeys:{},loadedKeys:{},requestedDate:'',activeView:'overview',filters:{},activeJobId:'',loadSeq:0};
  var titles={overview:'今日决策',hypotheses:'交易假设',candidates:'候选与拒绝',intraday:'盘中只读证据',portfolio:'目标组合',positions:'持仓与退出',orders:'模拟订单',validation:'回测验收',missed:'漏抓复盘',evidence:'数据与系统'};
  ...
  function fetchJson(path){return fetch(path,{headers:{Accept:'application/json'},cache:'no-store'}).then(function(r){if(!r.ok)return responseError(r,path);return r.json()})}
  ...
  function load(){
    ...
    var view=state.activeView||requestedView(),contextPath='/context'+(state.requestedDate?'?'+dateParam():''),requests=[
    ...
    if(view==='overview'){add('forecasts',api3(withDate('/forecasts/latest?limit=200')),[]);add('oversold',api3(withDate('/forecasts/latest?strategy_key=oversold_reversal&limit=200')),[]);add('targets',api3(withDate('/portfolio/latest')),[])}
    if(view==='hypotheses')add('hypotheses',api3(withDate('/hypotheses/latest?limit=300')),[]);
    if(view==='candidates')add('stockPool',api3(withDate('/stock-pool')),{});
    ...
    if(view==='positions'){add('positions',api2('/accounts/paper-main-v2/positions'),[]);add('targets',api3(withDate('/portfolio/latest')),[])}
    ...
    return Promise.all(requests.map(function(item){return requestResult(item[0],item[1],item[2])})).then(function(results){
      ...
      function finish(){if(loadSeq!==state.loadSeq)return;var failed=Object.keys(state.errors),stamp=new Date().toLocaleTimeString('zh-CN',{hour12:false});el('updatedAt').textContent=failed.length?'部分读取失败 · '+stamp:'刷新于 '+stamp;renderAll();activateView(view);el('truthContext').setAttribute('aria-busy','false');notifyParentResize()}
    ...
    }).catch(function(err){if(loadSeq!==state.loadSeq)return;state.errors.load=errorText(err);el('updatedAt').textContent='读取失败';renderTruthContext();el('heroTitle').textContent='接口读取失败';el('heroReason').textContent=errorText(err);el('hero').classList.add('blocked')})
  }
  function renderAll(){renderTruthContext();renderChrome();renderActions();renderOverview();renderThemeAudit();renderHypotheses();renderCandidates();renderIntraday();renderPortfolio();renderPositions();renderOrders();renderValidation();renderRecall();renderLearning();renderEvidence();renderLineage();renderBatchDiff();renderAdvisoryResearch();renderHorizonResearch();renderCounterfactualResearch();renderShadowGovernance()}
  function truthState(){
  ...
  function renderTruthContext(){
    ...
    var paperLabel=paperAllowed?'模拟可入队':truth.code==='LOADING'?'等待批次':truth.code!=='READY'?'不可入队':scope==='RESEARCH_ONLY'?'RESEARCH_ONLY':state.errors.readiness?'V2 权限不可用':'V2 复验阻断';
    setAxis('paperAuthority',paperAllowed?'allowed':scope==='RESEARCH_ONLY'&&truth.code==='READY'?'limited':'blocked',paperLabel,paperAllowed?'V2 仍会在成交前复验账户 / 现金 / T+1 / 行情':'研究排序绝不升级为可执行目标');
  ...
  function renderActions(){
    ...
    el('actionStatus').textContent=(gate.recovery?'当前批次不可用或已阻断；允许重跑仅用于恢复，仍不绕过 V2 执行门禁。 ':brief(daily,'日级')+'；')+'盘中扫描由受控调度运行，本页只读';
  }
  function renderChrome(){
    ...
    var truth=truthState();
    if(truth.code==='UNAVAILABLE'){el('heroTitle').textContent='数据不可用，不能形成交易结论';el('heroReason').textContent=truth.reason;el('hero').classList.add('blocked')}
    ...
    else if(truth.code==='READY'&&ready===false){el('heroTitle').textContent='研究决策有效，模拟执行门禁阻断';el('heroReason').textContent='研究真值保持 READY；V2 复验只影响模拟执行轴，不会改写研究结论。';el('hero').classList.add('blocked')}
    else if(!run.run_uid){el('heroTitle').textContent='V3 尚未产生首个决策';el('heroReason').textContent='完成样本外验收后才会启用模拟组合。';el('hero').classList.add('blocked')}
  ...
  function renderCandidates(){
    ...
    if(!el('candidateDate').value){var defaultDate=(state.stockPool||{}).trade_date||(state.context||{}).data_date||(state.overview.run||{}).trade_date||'';if(defaultDate)el('candidateDate').value=String(defaultDate).slice(0,10)}
    var sf=el('strategyFilter').value,st=el('statusFilter').value,q=el('candidateSearch').value.trim().toLowerCase();
    var source=((state.stockPool||{}).items||[]).map(function(item){var rejection=item.rejection||{},statuses=item.forecast_statuses||[],forecastStatus=item.target?'RESEARCH_TARGET':item.rejection?'PORTFOLIO_REJECTED':statuses[0]||(item.is_strategy_candidate?'RESEARCH_ONLY_UNCALIBRATED':'RESEARCH_SAMPLE');return Object.assign({},item,{short_name:item.stock_name,strategy_key:(item.strategy_keys||[])[0]||'',forecast_status:forecastStatus,reasons:[rejection.reason||rejection.reason_code].concat(item.reasons||[]).filter(Boolean)})});
    var rows=source.filter(function(x){return(!sf||(x.strategy_keys||[]).indexOf(sf)>=0)&&(!st||(x.forecast_statuses||[]).indexOf(st)>=0||x.forecast_status===st)&&(!q||String(x.stock_code).toLowerCase().indexOf(q)>=0||String(x.short_name).toLowerCase().indexOf(q)>=0||themeText(x).toLowerCase().indexOf(q)>=0)});
    ...
    function card(x){var calibrated=hasCalibratedExpectation(x),reasons=(x.reasons||[]).join('；')||status(x.forecast_status),fullTheme=themeText(x);return '<article class="candidate-card"><div class="candidate-card-head"><span class="candidate-rank">#'+esc(num(x.rank_no,0))+'</span>'+security(x.stock_code,x.short_name)+'<span class="badge '+(x.target?'safe':x.rejection?'danger':'warning')+'">'+esc(status(x.forecast_status))+'</span></div><div class="candidate-card-route"><strong>'+esc(strategyText(x))+'</strong><span title="'+esc(fullTheme)+'">'+esc(themeSummary(x))+'</span></div><div class="candidate-card-metrics"><div><span>综合分</span><strong>'+num(x.raw_score,3)+'</strong></div><div><span>研究净期望</span><strong class="'+(calibrated&&Number(x.expected_return_net_pct)>0?'safe-text':calibrated?'':'warning-text')+'">'+(calibrated?pct(x.expected_return_net_pct):'未校准')+'</strong></div><div><span>权限</span><strong class="warning-text">RESEARCH_ONLY</strong></div><div><span>批次</span><strong>'+esc(String((state.stockPool||{}).run_uid||'—').slice(0,10))+'</strong></div></div><p class="candidate-card-reason"><b>判断依据：</b>'+esc(reasons)+'</p></article>'}
    ...
    el('candidateRows').innerHTML=pageRows.length?pageRows.map(function(x){var calibrated=hasCalibratedExpectation(x);return '<tr><td>'+num(x.rank_no,0)+'</td><td>'+security(x.stock_code,x.short_name)+'</td><td>'+esc(strategyText(x))+'</td><td title="'+esc(themeText(x))+'">'+esc(themeSummary(x))+'</td><td>'+num(x.raw_score,3)+'</td><td class="'+(calibrated&&Number(x.expected_return_net_pct)>0?'safe-text':calibrated?'':'warning-text')+'">'+(calibrated?pct(x.expected_return_net_pct):'未校准')+'</td><td>'+(calibrated?pct(Number(x.probability_positive)*100,1):'未校准')+'</td><td>—</td><td>—</td><td>—</td><td>'+esc(status(x.forecast_status))+'</td><td class="reason">'+esc((x.reasons||[]).join('；'))+' · RESEARCH_ONLY</td></tr>'}).join(''):decisionEmptyFor('stockPool',12,'已验证批次在该筛选条件下没有记录');
  ...
  window.addEventListener('message',function(event){
    ...
    var incomingDate=String(event.data.requested_date||''),changed=incomingDate&&incomingDate!==state.requestedDate;state.requestedDate=incomingDate||state.requestedDate;applyFilters(event.data.filters||{});activateView(String(event.data.view||'overview'));if(changed||event.data.filters)load();notifyParentResize()
  });
  if(window.ResizeObserver)new ResizeObserver(notifyParentResize).observe(document.documentElement);
  var candidateTimer=null;
  function reloadCandidates(){state.candidatePage=1;var day=el('candidateDate').value;syncFilters();api3('/stock-pool'+(day?'?trade_date='+encodeURIComponent(day):'')).then(function(v){delete state.errors.stockPool;state.stockPool=unwrap(v)||{};renderCandidates();renderTruthContext();notifyParentResize()}).catch(function(err){state.errors.stockPool=errorText(err);renderCandidates();renderTruthContext();notifyParentResize()})}
