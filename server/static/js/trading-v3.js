(function(){
  ...
  function renderPositions(){var names={};state.forecasts.forEach(function(x){names[x.stock_code]=x.short_name});state.targets.forEach(function(x){names[x.stock_code]=x.short_name});el('positionRows').innerHTML=state.positions.length?state.positions.map(function(x){var source=x.ledger_source==='LEGACY_EVENT_SIM'?'事件模拟账本':'V2/V3账本',pnl=Number(x.unrealized_pnl),pnlClass=Number.isFinite(pnl)?(pnl>0?'safe-text':pnl<0?'danger-text':''):'',quote=x.current_price==null?'暂无行情':money(x.current_price),quoteAt=x.quote_at?String(x.quote_at)+' · '+sourceName(x.quote_source):'暂无行情时间';return '<tr><td>'+security(x.stock_code,x.short_name||names[x.stock_code])+'</td><td>'+esc(status(x.position_state||x.state))+'</td><td>'+num(x.remaining_quantity||x.quantity,0)+'</td><td>'+num(x.sellable_quantity,0)+'</td><td>'+money(x.cost_price||x.average_cost)+'</td><td>'+quote+'</td><td>'+money(x.market_value)+'</td><td class="'+pnlClass+'">'+money(x.unrealized_pnl)+'</td><td class="'+pnlClass+'">'+pct(x.unrealized_pnl_pct)+'</td><td>'+money(x.protective_stop)+'</td><td>'+num(x.add_count,0)+'</td><td>'+esc(quoteAt)+'</td><td class="reason">'+esc('['+source+'] '+(x.invalidation_condition||x.last_reason||'趋势、硬止损和净期望动态复核'))+'</td></tr>'}).join(''):empty(13,'当前统一模拟账本没有持仓；候选不是持仓，只有模拟成交后才会显示')}
