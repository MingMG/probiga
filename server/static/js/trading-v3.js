(function(){
  ...
  function renderCandidates(){
    ...
    el('candidateSummary').textContent=(formalCurrent?(pool.is_as_of_fallback===true?'VERIFIED COMPLETED · 最新成功批次 · ':'VERIFIED COMPLETED · 当前正式票池 · '):governanceDeferred?'治理延期 · 只读研究 · ':historicalFallback?'历史只读 · ':researchDefaultFallback?'研究观察 · ':'')+'请求日 '+requestedDate+' · 决策日 '+decisionDate+' · 数据日 '+dataDate+' · 执行日 '+(poolExecutionDate||'—')+' · 当前显示 '+rows.length+' 条'+(formalCurrent?'':' · 阻断 '+formalTruth.reasonCode);el('candidatePageStatus').textContent='第 '+state.candidatePage+' / '+pageCount+' 页';el('candidatePrev').disabled=state.candidatePage<=1;el('candidateNext').disabled=state.candidatePage>=pageCount;el('candidatePager').hidden=rows.length<=pageSize;
  }
