(function(){
  ...
  function shouldDisplayResearchPool(formalCurrent,pool,requestedDate){return formalCurrent!==true&&researchPoolIsReadable(pool,requestedDate)}
  ...
  function load(){
    ...
    return runRequestQueue(bootstrapRequest,1).then(function(results){
      ...
      if(view==='candidates'){
        var researchRequestDate=String(state.requestedDate||(daily.context||{}).requested_date||(daily.context||{}).requested_trade_date||(daily.stock_pool||{}).requested_trade_date||(daily.context||{}).decision_session_date||(daily.stock_pool||{}).trade_date||'').slice(0,10);
  ...
  function renderCandidates(){
    ...
    var researchDisplay=renderResearchObservationPool(formalCurrent,state.requestedDate||pool.requested_trade_date||state.researchStockPoolRequestDate),researchPoolVisible=researchDisplay.visible===true;
    ...
    var researchNotice=el('candidateResearchNotice');if(researchNotice){researchNotice.hidden=formalCurrent&&!researchDefaultFallback;researchNotice.textContent=researchPoolVisible?'请求日正式票池尚未生成，先展示同日研究观察票池；这些股票尚未满足正式买入条件。':governanceDeferred?'DEFERRED_DB / 治理数据库迁移延期：策略池仅保留不可变审计行，全部为 RESEARCH_ONLY；当前不展示可执行买卖区间或止损指令，也不会创建模拟或真实订单。':!formalCurrent&&!historicalFallback?'RESEARCH_ONLY / 正式票池不可用：'+formalTruth.reason+'。旧日期、未验证或研究只读候选不会进入当前可执行展示。':researchDefaultFallback?'本批次没有“允许买入区间、等待触发或仅模拟研究”候选；当前按策略排名展示研究观察股票。它们没有可执行资格，不会创建模拟或真实订单。':''}
  ...
  function reloadCandidates(){state.candidatePage=1;state.researchCandidatePage=1;state.researchStockPool={};state.researchStockPoolRequestDate='';var day=el('candidateDate').value;if(day)state.requestedDate=day;syncFilters();load()}
  function refreshCandidatePools(){
    ...
    var target=String(state.requestedDate||el('candidateDate').value||(state.stockPool||{}).requested_trade_date||'').slice(0,10);if(!/^\d{4}-\d{2}-\d{2}$/.test(target))return;
    var refreshSeq=++state.candidateRefreshSeq,loadSeq=state.loadSeq;state.candidateRefreshPending=true;state.researchStockPoolRequestDate=target;
    Promise.all([
      requestResult('candidateDailyResult',api3('/daily-result?trade_date='+encodeURIComponent(target),60000),{}),
      requestResult('candidateResearchPool',api3('/research/stock-pool?trade_date='+encodeURIComponent(target),60000),{}),
      requestResult('candidateDailyDelivery',fetchJson('/api/scheduler/daily-delivery?trade_date='+encodeURIComponent(target)),{})
    ]).then(function(results){
      if(refreshSeq!==state.candidateRefreshSeq||loadSeq!==state.loadSeq||target!==String(state.requestedDate||el('candidateDate').value||'').slice(0,10))return;
      var dailyResult=results[0],researchResult=results[1],deliveryResult=results[2];
      if(dailyResult.ok){var daily=dailyResult.value||{},dailyDate=String(((daily.context||{}).requested_date)||((daily.context||{}).requested_trade_date)||((daily.stock_pool||{}).requested_trade_date)||((daily.stock_pool||{}).trade_date)||'').slice(0,10);if(daily.context&&daily.overview&&daily.stock_pool&&(!dailyDate||dailyDate===target)){state.dailyResult=daily;state.context=daily.context;state.overview=daily.overview;state.stockPool=daily.stock_pool;delete state.errors.candidateDailyRefresh}else state.errors.candidateDailyRefresh='每日原子结果与当前请求日不一致'}else state.errors.candidateDailyRefresh=dailyResult.error;
      if(researchResult.ok&&String((researchResult.value||{}).trade_date||'').slice(0,10)===target){state.researchStockPool=researchResult.value||{};delete state.errors.researchStockPool}else{state.researchStockPool={};state.errors.researchStockPool=researchResult.ok?'研究观察池与当前请求日不一致':researchResult.error}
