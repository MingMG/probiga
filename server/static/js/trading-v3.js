(function(){
  ...
  function renderCandidates(){
    ...
    var pool=state.stockPool||{},historicalFallback=pool.is_historical_fallback===true,governanceDeferred=pool.governance_deferred===true||pool.activation_enabled===false;
    var source=(pool.items||[]).map(function(item){var rejection=item.rejection||{},actionability=governanceDeferred?'RESEARCH_ONLY':String(item.actionability||((item.action_plan||{}).actionability)||'RESEARCH_ONLY');return Object.assign({},item,{short_name:item.stock_name,strategy_key:(item.strategy_keys||[])[0]||'',actionability:actionability,forecast_status:actionability,reasons:[rejection.reason||rejection.reason_code].concat(item.reasons||[]).filter(Boolean)})}).filter(function(item){return item.is_strategy_candidate===true||(st==='REJECTED'&&item.actionability==='REJECTED')});
    ...
    var researchNotice=el('candidateResearchNotice');if(researchNotice){researchNotice.hidden=!governanceDeferred&&!researchDefaultFallback;researchNotice.textContent=governanceDeferred?'DEFERRED_DB / 治理数据库迁移延期：策略池仅保留不可变审计行，全部为 RESEARCH_ONLY；当前不展示可执行买卖区间或止损指令，也不会创建模拟或真实订单。':researchDefaultFallback?'本批次没有“允许买入区间、等待触发或仅模拟研究”候选；当前按策略排名展示研究观察股票。它们没有可执行资格，不会创建模拟或真实订单。':''}
    var poolStats=el('candidatePoolStats');if(poolStats)poolStats.innerHTML='<span><b>'+Number(summary.strategy_candidate_count||0)+'</b>研究候选</span><span><b>'+Number(governanceDeferred?0:summary.wait_trigger_count||0)+'</b>等待触发</span><span><b>'+Number(summary.target_count||0)+'</b>研究目标（不可直接下单）</span><span><b>'+Number(summary.rejected_count||0)+'</b>明确拒绝</span>';
    el('candidateSummary').textContent=(governanceDeferred?'治理延期 · 只读研究 · ':historicalFallback?'历史只读 · ':researchDefaultFallback?'研究观察 · ':'')+'请求日 '+requestedDate+' · 决策日 '+decisionDate+' · 数据日 '+dataDate+' · 当前显示 '+rows.length+' 条';el('candidatePageStatus').textContent='第 '+state.candidatePage+' / '+pageCount+' 页';el('candidatePrev').disabled=state.candidatePage<=1;el('candidateNext').disabled=state.candidatePage>=pageCount;el('candidatePager').hidden=rows.length<=pageSize;
  }
