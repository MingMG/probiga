(function(){
  ...
  function stockPoolWithHistoricalFallback(requestedDate){
    ...
    return api3(exactPath).then(function(payload){
      var exact=unwrap(payload)||{};
      if(!target||exact.run_uid)return Object.assign({},exact,{requested_trade_date:target||String(exact.decision_session_date||exact.trade_date||'').slice(0,10),is_historical_fallback:false});
      function missingExact(){return Object.assign({},exact,{requested_trade_date:target,exact_run_missing:true,is_historical_fallback:false})}
      return api3('/stock-pool').then(function(latestPayload){
        var latest=unwrap(latestPayload)||{},latestSession=String(latest.decision_session_date||latest.trade_date||'').slice(0,10);
        if(!latest.run_uid||!latestSession||latestSession>=target)return missingExact();
        return Object.assign({},latest,{requested_trade_date:target,exact_run_missing:true,is_historical_fallback:true,historical_fallback_session_date:latestSession,historical_fallback_reason:'请求日没有 V3 决策批次，展示此前最近一次可读历史策略池'});
      }).catch(missingExact)
    })
  }
  ...
  function load(){
    ...
    if(view==='candidates')add('stockPool',stockPoolWithHistoricalFallback(state.requestedDate),{});
  ...
  function renderCandidates(){
    ...
    el('candidateRows').innerHTML=pageRows.length?pageRows.map(function(x){var calibrated=hasCalibratedExpectation(x),plan=plans(x),actionLabel=actionabilityNames[plan.actionability]||plan.actionability;if(historicalFallback)actionLabel='历史只读 · 原'+actionLabel;return '<tr><td>'+num(x.rank_no,0)+'</td><td>'+security(x.stock_code,x.short_name)+'</td><td>'+esc(strategyText(x))+'</td><td title="'+esc(themeText(x))+'">'+esc(themeSummary(x))+'</td><td>'+esc(plan.buy)+'</td><td>'+esc(plan.sell)+'</td><td>'+esc(plan.emergency)+'</td><td>'+num(x.raw_score,3)+'</td><td class="'+(calibrated&&Number(x.expected_return_net_pct)>0?'safe-text':calibrated?'':'warning-text')+'">'+(calibrated?pct(x.expected_return_net_pct):'未校准')+'</td><td>'+(calibrated?pct(Number(x.probability_positive)*100,1):'未校准')+'</td><td>'+esc(calibrated?ratio(x.profit_factor):'未校准')+'</td><td>'+esc(calibrated?ratio(x.payoff_ratio):'未校准')+'</td><td>'+esc(calibrated?num(x.sample_count,0):'0')+'</td><td>'+esc(actionLabel)+'</td><td class="reason">'+esc((x.reasons||[]).join('；'))+' · V3_IMMUTABLE_RUN'+(historicalFallback?' · HISTORICAL_READ_ONLY':'')+'</td></tr>'}).join(''):decisionEmptyFor('stockPool',15,candidateEmptyText);
    ...
    var historyNotice=el('candidateHistoryNotice');if(historyNotice){historyNotice.hidden=!historicalFallback;historyNotice.textContent=historicalFallback?'请求日 '+requestedDate+' 没有 V3 决策批次；当前展示最近一次可读历史策略池（决策日 '+decisionDate+'，数据日 '+dataDate+'）。这些股票仅供历史复核，全部不可执行，也不会创建模拟或真实订单。':''}
    var researchNotice=el('candidateResearchNotice');if(researchNotice){researchNotice.hidden=!researchDefaultFallback;researchNotice.textContent=researchDefaultFallback?'本批次没有“允许买入区间、等待触发或仅模拟研究”候选；当前按策略排名展示研究观察股票。它们没有可执行资格，不会创建模拟或真实订单。':''}
    var poolStats=el('candidatePoolStats');if(poolStats)poolStats.innerHTML='<span><b>'+Number(summary.strategy_candidate_count||0)+'</b>研究候选</span><span><b>'+Number(summary.wait_trigger_count||0)+'</b>等待触发</span><span><b>'+Number(summary.target_count||0)+'</b>研究目标（不可直接下单）</span><span><b>'+Number(summary.rejected_count||0)+'</b>明确拒绝</span>';
    el('candidateSummary').textContent=(historicalFallback?'历史只读 · ':researchDefaultFallback?'研究观察 · ':'')+'请求日 '+requestedDate+' · 决策日 '+decisionDate+' · 数据日 '+dataDate+' · 当前显示 '+rows.length+' 条';el('candidatePageStatus').textContent='第 '+state.candidatePage+' / '+pageCount+' 页';el('candidatePrev').disabled=state.candidatePage<=1;el('candidateNext').disabled=state.candidatePage>=pageCount;el('candidatePager').hidden=rows.length<=pageSize;
  }
